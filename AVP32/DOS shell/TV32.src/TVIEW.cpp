...
size_t TView::dataSize()
{
    return 0;
}

inline void set_commandSetChanged(Boolean condition) {
  TView::commandSetChanged = condition;
  if ( TView::commandSetChanged ) TProgram::set_event_delay(0);
}

void TView::disableCommands( TCommandSet& commands )
{
    set_commandSetChanged(Boolean( commandSetChanged ||
...
{
    if( (mode & dmDragMove) != 0 && (getShiftState() & 3) == 0 )
        p += delta;
    else if( (mode & dmDragGrow) != 0 && (getShiftState() & 3) != 0 )
        s += delta;
}
