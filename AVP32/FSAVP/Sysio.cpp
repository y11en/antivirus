...
extern KUstring TempPath;
extern KInterlockedCounter InMyOpenClose;
...
static KMutex AccessArrayMutex(3);
static CPtrArray* InternalAccessArray=NULL;

int InternalAccessAdd(const char* name)
{
	if(!InternalAccessArray || name==NULL) return -1;
	char* newbuf=new char[strlen(name)+5];
	if(newbuf==NULL)return -1;
	*(DWORD*)newbuf=-1;
	strcpy(newbuf+4,name);
	...
	_FSAllowInternalAccess(np);
#else
	_FSAllowInternalAccess((PUCHAR)np);
#endif

#endif

	AccessArrayMutex.Wait();
	int r=InternalAccessArray->Add(newbuf);
	AccessArrayMutex.Release();

	return r;
}

static int InternalAccessHandleSetAt(int i, DWORD handle)
{
	int r=-1;
	char* buf;
	AccessArrayMutex.Wait();
	if(!InternalAccessArray || InternalAccessArray->GetUpperBound()<i ) goto ret;
	buf=(char*)InternalAccessArray->GetAt(i);
...
static int InternalAccessRemoveAt(int i)
{
	int r=-1;
	char* buf;
	AccessArrayMutex.Wait();
	if(!InternalAccessArray || InternalAccessArray->GetUpperBound()<i )goto ret;
	buf=(char*)InternalAccessArray->GetAt(i);
	InternalAccessArray->RemoveAt(i);
	if(buf)
	{
#if NTVERSION>=400
		const char* np=buf+4;
...
#if GATEKEEPER_API_VERSION!=0x0100
		_FSRemoveInternalAccess((PCHAR)np);
#else
		_FSRemoveInternalAccess((PUCHAR)np);
#endif
...
void InternalAccessRemoveAll()
{
	if(!InternalAccessArray)return;
	AccessArrayMutex.Wait();
	for(int i=InternalAccessArray->GetUpperBound();i>=0;i--)
	{
		AccessArrayMutex.Release();
		InternalAccessRemoveAt(i);
		AccessArrayMutex.Wait();
	}
	AccessArrayMutex.Release();
}

void InternalAccessDestroy()
{
	InternalAccessRemoveAll();
	AccessArrayMutex.Wait();
	if(InternalAccessArray){
		delete InternalAccessArray;
		InternalAccessArray=0;
	}
	AccessArrayMutex.Release();
}

BOOL InternalAccessInit()
{
#if NTVERSION>=400
	if(InternalAccessArray)return TRUE;
	AccessArrayMutex.Wait();
	InternalAccessArray=new CPtrArray;
	AccessArrayMutex.Release();
#endif
	return !!InternalAccessArray;
}
...
{
		int n=InternalAccessAdd(lpFileName);
...
//		if(dwShareMode & FILE_SHARE_WRITE) dwShareMode|=FILE_SHARE_DELETE;
		...
		if(r!=INVALID_HANDLE_VALUE)
		{
			...
			if(LimitCompoundSize){
				if( dwDesiredAccess==GENERIC_READ && (_AvpGetFileSize(r,NULL) >= LimitCompoundSize)){
					if(so_ptr) so_ptr->MFlags&=~(MF_ARCHIVED|MF_PACKED|MF_MAILBASES|MF_MAILPLAIN);
					_DebugTrace(TraceInfo,"Change MFlags by limitcompoundsize");
				}
			}
		...
		if(r==INVALID_HANDLE_VALUE)
			InternalAccessRemoveAt(n);
		else 
			InternalAccessHandleSetAt(n, (DWORD)r);
        ...
        )
{
	...
	InternalAccessRemove((DWORD)hFile);
...
//return FALSE;
        ...
        // Convert it back to unicode.
        //
        RtlInitAnsiString( &ansiFilename, lpFileName );
        RtlAnsiStringToUnicodeString( &uniFilename, &ansiFilename, TRUE);
