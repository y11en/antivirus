...
begin
  ...
  case FFilterEditMode of
   femNumber : begin
                DeleteFilterItems;
                AddFilterItem('Равно (=)',     fetNumberList,     '{FLD} IN ({X})',     1, 'Допустим список значений через запятую');
                AddFilterItem('Не равно (<>)', fetNumberList,     '{FLD} NOT IN ({X})', 1, 'Допустим список значений через запятую');
                AddFilterItem('Больше (>)',    fetNumber,         '{FLD} > {X}',        1);
                AddFilterItem('Меньше (<)',    fetNumber,         '{FLD} < {X}',        1);
                AddFilterItem('В диапазоне',   fetNumberDiapazon, '{FLD} BETWEEN {X1} AND {X2}',     2);
                AddFilterItem('Вне диапазона', fetNumberDiapazon, '{FLD} NOT BETWEEN {X1} AND {X2}', 2);
                if FFilterItemsNull then begin
                 AddFilterItem('Заполнено',   fetNumber, '{FLD} IS NOT NULL', 0 + 4);
                 AddFilterItem('Не заполнено', fetNumber, '{FLD} IS NULL', 0 + 4);
                end;
               end;
   femNumberCode : begin
                DeleteFilterItems;
   ...
   femString : begin
                DeleteFilterItems;
                AddFilterItem('Содержит',             fetString, 'UPPER({FLD}) LIKE UPPER(''%''||''{X}''||''%'')', 1);
                AddFilterItem('Не содержит',          fetString, 'UPPER({FLD}) NOT LIKE UPPER(''%''||''{X}''||''%'')', 1);
                AddFilterItem('Начинается с ...',     fetString, 'UPPER({FLD}) LIKE UPPER(''{X}''||''%'')', 1);
                AddFilterItem('Заканчивается на ...', fetString, 'UPPER({FLD}) LIKE UPPER(''%''||''{X}'')', 1);
                AddFilterItem('Равно',                fetString, 'UPPER({FLD}) LIKE UPPER(''{X}'')', 1);
                AddFilterItem('Не равно',             fetString, 'UPPER({FLD}) LIKE UPPER(''{X}'')', 1);
                if FFilterItemsNull then begin
                 AddFilterItem('Заполнено',   fetString, 'LTRIM({FLD}) IS NOT NULL', 4);
                 AddFilterItem('Не заполнено', fetString, 'LTRIM({FLD}) IS NULL', 4);
                end;
               end;
   femDate : begin
                DeleteFilterItems;
                AddFilterItem('Равно (=)',     fetDate, 'TRUNC({FLD}) =  TO_DATE(''{X}'',''DD.MM.YYYY'')', 1, 'Формат даты ДД.ММ.ГГГГ');
                AddFilterItem('Не равно (<>)', fetDate, 'TRUNC({FLD}) != TO_DATE(''{X}'',''DD.MM.YYYY'')', 1, 'Формат даты ДД.ММ.ГГГГ');
                AddFilterItem('Больше (>)',    fetDate, 'TRUNC({FLD}) > TO_DATE(''{X}'',''DD.MM.YYYY'')', 1);
                AddFilterItem('Меньше (<)',    fetDate, 'TRUNC({FLD}) < TO_DATE(''{X}'',''DD.MM.YYYY'')', 1);
                AddFilterItem('В диапазоне',   fetDateDiapazon, 'TRUNC({FLD}) BETWEEN TO_DATE(''{X1}'',''DD.MM.YYYY'') AND TO_DATE(''{X2}'',''DD.MM.YYYY'')', 2);
                AddFilterItem('Вне диапазона', fetDateDiapazon, 'TRUNC({FLD}) NOT BETWEEN TO_DATE(''{X1}'',''DD.MM.YYYY'') AND TO_DATE(''{X2}'',''DD.MM.YYYY'')', 2);
                if FFilterItemsNull then begin
                 AddFilterItem('Заполнено',   fetDate, '{FLD} IS NOT NULL', 4);
                 AddFilterItem('Не заполнено', fetDate, '{FLD} IS NULL', 4);
                end;
               end;
   femCustom : begin
                 DeleteFilterItems;
...
begin
 try
   ...
   // Тип фильтрации
   if (TekFilterItem = nil) then  begin
    ErrorMessage := 'Не задан тип фильтрации';
    exit;
   end;
   ...
   if TekFilterItem.EditType = fetString then begin
    if not(Trim(FilterText1) <> '') then begin
     ErrorMessage := 'Не задан образец поиска';
     exit;
    end;
   end;
   // Одно число
   ...
   if TekFilterItem.EditType = fetNumberList then begin
    if not CheckNumberValues(FilterText1) then begin
     ErrorMessage := 'Некорректное число или список чисел';
     exit;
    end;
   end;
   // Диапазон чисел
   if TekFilterItem.EditType = fetNumberDiapazon then begin
    Res := true;
    if not CheckNumberValue(FilterText1) then begin
     ErrorMessage := 'Некорректная начальная граница диапазона';
     Res := false;
    end else FEdit1OK := true;
    if not CheckNumberValue(FilterText2) then begin
     ErrorMessage := 'Некорректная конечная граница диапазона';
     Res := false;
    end else FEdit2OK := true;
    if not(res) then exit;
    if StrToFloat(Trim(FilterText1)) > StrToFloat(Trim(FilterText2)) then begin
     ErrorMessage := 'Конечная граница должна быть больше начальной';
     FEdit1OK := false; FEdit2OK := false;
    end;
   end;
   // Одна дата
   ...
   if TekFilterItem.EditType = fetDateDiapazon then begin
    if not CheckDateValue(FilterText1) then begin
     ErrorMessage := 'Некорректная дата начала периода';
     exit;
    end else FEdit1OK := true;
    if not CheckDateValue(FilterText2) then begin
     ErrorMessage := 'Некорректная дата конца периода';
     exit;
    end else FEdit2OK := true;
   end;
   Result := true;
