...
NTSTATUS zDeleteFile(IN PUNICODE_STRING pusFileName)
{
	__try {
		...
		// Подготовка атрибутов объекта "Файл"
		OBJECT_ATTRIBUTES ob;
	...
	__try {
		...
		__try {
			...
			do {
				...
				// Запись считанных данных в целевой файл
			...
			// Если код ошибки = "конец файла", то копирование закончилось успешно
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT pDriverObject, IN PUNICODE_STRING pusRegistryPath)
{
	...
	// Выполнение операций удаления служб и жрайверов
	ExecuteDeleteDrivers(pusRegistryPath, TRUE);
	// Выполнение операций на уровне реестра
	ExecuteREGOper(pusRegistryPath, TRUE);

	
	// ************* Удаление ключа драйвера целиком ************** 
	...
	// Возврат ошибки, чтобы драйвер выгрузился из памяти
