...
var
 ...
 begin
   ...
   if not(CompareMem(@buf, @SIGN_MSOffice, 20)) and not(CompareMem(@buf, @SIGN_MacOffice, 20)) then begin
    ...
    AddDebug('Пропуск блока 20 байт - "магический блок"');
   Result := true;
 end;
 ...
 begin
  ...
  while true do begin
   ...
   AddDebug('-->Считано имя '+copy(name, 1, len div 2));
...
begin
 ...
 AddDebug('Сигнатура 1 = '+IntToHex(VBAProjectHeader.Sign, 2));
 AddDebug('Сигнатура 2 = '+IntToHex(VBAProjectHeader.Sign1, 2));
 AddDebug('Кодовая страница = '+IntToStr(VBAProjectHeader.CodePage));
 ...
 AddDebug('Количество записей в VBA проекте = '+IntToStr(RecordCount));
 ...
 for i := 0 to RecordCount-1 do begin
  ...
  FS.Read(CodeRel, 4);
  AddDebug('   Смещение = '+IntToStr(CodeRel));
  FS.Seek(2, soFromCurrent);
