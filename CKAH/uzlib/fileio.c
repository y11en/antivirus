...
#  define WriteError(buf,len,strm) \
   (win_fprintf(pG, strm, (unsigned int)len, (char far *)buf) != (int)(len))
#else /* !WINDLL */
#  ifdef USE_FWRITE
#    define WriteError(buf,len,strm) \
...
#else /* !USE_STRM_INPUT */
# ifdef O_BINARY
    G.zipfd = _open(G.zipfn, O_RDONLY | O_BINARY);
# else
...
#endif /* UNIXBACKUP */
        {
            ...
            if (!(G.statbuf.st_mode & S_IWRITE)) {
                Trace((stderr,
                  ...
                  FnFilter1(G.filename)));
                _chmod(G.filename, S_IREAD | S_IWRITE);
    ...
    register unsigned size;
{
    ...
    while (size) {
        if (G.incnt <= 0) {
            if ((G.incnt = _read(G.zipfd, (char *)G.inbuf, INBUFSIZ)) == 0)
                return (n-size);
            else if (G.incnt < 0) {
...
#else /* !USE_STRM_INPUT */
        G.cur_zipfile_bufstart = _lseek(G.zipfd, (LONGINT)bufstart, SEEK_SET);
#endif /* ?USE_STRM_INPUT */
        Trace((stderr,
                    ...
                    case 2:
                        {
                            ...
                            G.VMS_line_length -= (unsigned)remaining;
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    case SKIP:
        ...
        seek_zipf(__G__ (LONGINT)(G.cur_zipfile_bufstart - G.extra_bytes +
