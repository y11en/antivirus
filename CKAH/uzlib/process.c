...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (G.ziplen <= INBUFSIZ) {
        _lseek(G.zipfd, 0L, SEEK_SET);
        if ((G.incnt = _read(G.zipfd,(char *)G.inbuf,(unsigned int)G.ziplen))
            == (int)G.ziplen)
...
#else /* !USE_STRM_INPUT */
            G.cur_zipfile_bufstart = _lseek(G.zipfd, G.ziplen-tail_len,
              SEEK_SET);
#endif /* ?USE_STRM_INPUT */
            if ((G.incnt = _read(G.zipfd, (char *)G.inbuf,
    ...
    /*-----------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
        ...
        for (i = 1;  !found && (i <= numblks);  ++i) {
            ...
            _lseek(G.zipfd, G.cur_zipfile_bufstart, SEEK_SET);
            if ((G.incnt = _read(G.zipfd,(char *)G.inbuf,INBUFSIZ))
                != INBUFSIZ)
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    G.real_ecrec_offset = (LONGINT)(G.cur_zipfile_bufstart + (G.inptr-G.inbuf));
#ifdef TEST
