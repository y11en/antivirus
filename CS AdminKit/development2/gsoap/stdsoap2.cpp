...
//#define KLCS_MODULENAME L"gsoap"
...
{ register int nwritten;
#if defined(__cplusplus) && !defined(WITH_LEAN) /* Change by Damir */ && !defined(SOAP_WITHOUT_STL) 
  if (soap->os)
  { soap->os->write(s, n);
    if (soap->os->good())
      return SOAP_OK;
 ...
 #else
         nwritten = send(soap->socket,(char *) s, n, soap->socket_flags);
 #endif        
#else
...
#endif  // ifdef KL_STDOUT_TRACE_ON   
    }
    n -= nwritten;
    s += nwritten;
  }
...
#ifndef PALM_1
#ifdef _WIN32
__forceinline soap_wchar __fastcall soap_get_andkaz(struct soap *soap)
#else
inline soap_wchar soap_get_andkaz(struct soap *soap)
#endif
{ register soap_wchar c;
  c = soap->ahead;
  if (c)
...
{ 
//KL_TMEASURE_BEGIN(L"soap_getbase64", 1)
#ifdef WITH_FAST
  soap->labidx = 0;
  for (;;)
  { register int i, k;
  ...
  { register int i;
    ...
    { register unsigned long m = 0;
      register int j = 0;
      do
      { register soap_wchar c = soap_get_andkaz(soap);
        if (c == '=' || c < 0)
        { unsigned char *p;
          i *= 3;
          switch (j)
	    ...
	    *n = (int)soap_size_block(soap, i);
          p = (unsigned char*)soap_save_block(soap, NULL, 0);
          if (c >= 0)
          { while ((int)((c = soap_get_andkaz(soap)) != EOF) && c != SOAP_LT && c != SOAP_TT)
              ;
	  }
          soap_unget(soap, c);
          return p;
...
/* ]WR */
  ...
  #if defined(N_PLAN_NLM) || defined(__unix)
        if (!getsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum, &n) && !soap->errnum)
  #elif defined(__unix)
        if (!getsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum,(socklen_t *) &n) && !soap->errnum)
  #else
        if (!getsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum,(int*) &n) && !soap->errnum)
  #endif
          break;
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not connect to host\n"));
        soap_set_sender_error(soap, tcp_error(soap), "TCP connect failed in tcp_connect()", SOAP_TCP_ERROR);
...
{ int fd;
 ...
 #else
  fd = (int)accept((SOAP_SOCKET)s, a, (int *)n);
 #endif 
#endif
#ifdef SOCKET_CLOSE_ON_EXEC
#ifdef WIN32
  ...
  { for (;;)
    { 
      ...
      { struct timeval timeout;
        ...
        { 
#ifdef N_PLAT_NLM
   int r = select((SOAP_SOCKET)(soap->master + 1), &fd, NULL, NULL, &timeout);
#else 
...
{ soap->version = 0;
  ...
  soap->float_format = "%.12g"; /* .8 preserves single FP precision as much as possible, but might not be very efficient */
  soap->double_format = "%.20lg"; /* .17 preserves double FP precision as much as possible, but might not be very efficient */
