...
/*KLCSAK_BEGIN_PRIVATE*/
    const time_t c_tmInvalidTime = c_invalid_time_t;
/*KLCSAK_END_PRIVATE*/
...
/*KLCSAK_BEGIN_PRIVATE*/
    // !!! size of this structure depends on compiler version !!! 
    struct precise_time_t
    {
        precise_time_t(time_t s, long ms = 0)
            :   sec(s)
            ,   msec(ms)
        {;};
        precise_time_t(old_precise_time_t x)
        {
            sec = (time_t)x.sec;
            msec = (long)x.msec;
        };
        ...
        static precise_time_t Now()
        {
            precise_time_t tmNow;
            klstd_time_t    sec = c_tmInvalidSysTime;
            unsigned        msec = 0;
            KLSTD_GetSystemTime(&sec, &msec);
            tmNow.sec = (time_t)sec;
            tmNow.msec = unsigned(msec);
            return tmNow;
        };
        ...
        operator old_precise_time_t() const 
        {
            return old_precise_time_t(
                            (klstd_old_time_t)sec,
                            (long)msec);
        };
