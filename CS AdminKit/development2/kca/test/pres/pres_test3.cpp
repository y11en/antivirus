// Test.cpp: implementation of the CTest3 class.
//
//////////////////////////////////////////////////////////////////////
...
const wchar_t TEST_EVENT_TYPE_1[] = L"TEST_EVENT_TYPE_1(some text on english)";
...
std::wstring CTest3::CreateEventType(const int n)
{
	wchar_t szName[100];
...
void CTest3::AddSubscriptions(EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
	...
	si.arEventParametersSortMask.push_back(EVENT_CLASS_MASK_EVENT_TYPE);
	si.arEventParametersSortMask.push_back(EVENT_CLASS_MASK_EVENT_TIME);
	si.arEventParametersSortMask.push_back(EVENT_CLASS_MASK_EVENT_PARAMS + std::wstring(L"/") + EVENT_BODY_PARAM_NAME);
	si.arEventParametersSortMask.push_back(EVENT_CLASS_MASK_EVENT_PARAMS + std::wstring(L"/") + EVENT_BODY_PARAM_PARAM + std::wstring(L"/") + EVENT_BODY_PARAM_PARAM_TEXT);

// Persist subscribe on all publishers and all events persist
	si.isSubscriptionPersist = true;
	si.sEventsListFileName = std::wstring(EVENTS_STORAGE_FOLDER_W) + L"\\Events_File_1_" + KLSTD_CreateGUIDString() + L".txt";
...
// Persist subscribe on all publishers and all events non persist
	si.isSubscriptionPersist = true;
	sID.clear();// = L"";
	si.sEventsListFileName = std::wstring(EVENTS_STORAGE_FOLDER_W) + L"\\Events_File_2_" + KLSTD_CreateGUIDString() + L".txt";
	pEvents->StartSubscription(si, sID);
	arSubscriptionIDs.push_back(sID);
...
// Persist subscribe on all publishers and 2 events persist
	si.isSubscriptionPersist = true;
	sID.clear();// = L"";
	si.sEventsListFileName = std::wstring(EVENTS_STORAGE_FOLDER_W) + L"\\Events_File_3_" + KLSTD_CreateGUIDString() + L".txt";
	pEvents->StartSubscription(si, sID);
	arSubscriptionIDs.push_back(sID);
...
// Non persist subscribe on the publisher and 2 events persist
	...
	sID.clear();// = L"";
	si.sEventsListFileName = std::wstring(EVENTS_STORAGE_FOLDER_W) + L"\\Events_File_4_" + KLSTD_CreateGUIDString() + L".txt";
	pEvents->StartSubscription(si, sID);
	arSubscriptionIDs.push_back(sID);
...
// Non persist subscribe on the publisher and 2 events non persist
	si.isSubscriptionPersist = false;
	sID.clear();// = L"";
	si.sEventsListFileName = std::wstring(EVENTS_STORAGE_FOLDER_W) + L"\\Events_File_5_" + KLSTD_CreateGUIDString() + L".txt";
	pEvents->StartSubscription(si, sID);
	arSubscriptionIDs.push_back(sID);
}

void CTest3::VerifyAddSubscriptions(EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
...
void CTest3::AddEvents(EventsStorage* pEvents, bool bEventType1, KLPRCI::ComponentInstance* pComponent, std::vector<std::wstring>& arSubscriptionIDs)
{
...
void CTest3::VerifyAddEvents(EventsStorage* pEvents, bool bEventType1, KLPRCI::ComponentInstance* pComponent, std::vector<std::wstring>& arSubscriptionIDs)
{
...
void CTest3::RemoveExpiredEvents(EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
	for(std::vector<std::wstring>::iterator i = arSubscriptionIDs.begin();i != arSubscriptionIDs.end();i++)
	{
...
void CTest3::RemoveAllEventsAndValidate(EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
	for(std::vector<std::wstring>::iterator i = arSubscriptionIDs.begin();i != arSubscriptionIDs.end();i++)
	{
...
void CTest3::RemoveEvents(std::wstring sSubscriber, bool bUseClearEventsFunction, EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
...
void CTest3::RemoveSubscriptionsAndVerify(EventsStorage* pEvents, std::vector<std::wstring>& arSubscriptionIDs)
{
	for(std::vector<std::wstring>::iterator i = arSubscriptionIDs.begin();i != arSubscriptionIDs.end();i++)
	{
...
//	pEvents->PauseSubscription(arSubscriptionIDs[0]);
