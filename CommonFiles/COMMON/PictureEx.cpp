...
CPictureEx::CPictureEx()
{
	...
	m_hFirstPictureReady = CreateEvent(NULL, FALSE, FALSE, NULL);
}
...
CPictureEx::~CPictureEx()
{
	...
	CloseHandle(m_hFirstPictureReady);
}
	...
	else
	{
		...
		// then store separate frames in an array
				...
				&frame.m_frameOffset, &frame.m_nDisposal) )
			{
				...
				if (OleLoadPicture(pStream,nBlockLen,TRUE/*BUG 10176 FIX*/,
					IID_IPicture,
...
BOOL CPictureEx::Draw()
{
	...
	// we'll start the thread that will handle it for us
		...
		if (m_hThread != NULL)
		{
			TRACE(_T("Draw: Couldn't start a GIF animation thread, already running\n"));
			return FALSE;
		}
		...
		WaitForSingleObject(m_hFirstPictureReady, INFINITE);
	} 
	else
	{
		if (m_pPicture)
...
BOOL CPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	...
	ASSERT(szResourceType);
     ...
     HINSTANCE hInst = AfxFindResourceHandle ( szResourceName,szResourceType );
	...
	if (!pDest)
	{
		TRACE(_T("Load (resource): Error locking global memory\n"));
		GlobalFree(hGlobal);
		FreeResource(hResData);
		return FALSE;
	}
	...
	if (!pSrc)
	{
		TRACE(_T("Load (resource): Error locking resource\n"));
		GlobalUnlock(hGlobal);
		GlobalFree(hGlobal);
		FreeResource(hResData);
		return FALSE;
...
BOOL CPictureEx::PrepareDC(int nWidth, int nHeight)
{
	...
	FillRect(m_hMemDC,&rect,(HBRUSH)(COLOR_WINDOW+1));
...
void CPictureEx::OnSysColorChange()
{
