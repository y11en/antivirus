...
IMPLEMENT_DYNAMIC(CResizableSheet, CBasePropertySheet)

BEGIN_MESSAGE_MAP(CResizableSheet, CBasePropertySheet)
...
{
	if (CBasePropertySheet::OnCreate(lpCreateStruct) == -1)
		return -1;
	...
	GetClientRect(&rect);
	// set resizable style
	ModifyStyle(DS_MODALFRAME, WS_CHILD | WS_CLIPCHILDREN );
...
{
	BOOL bResult = CBasePropertySheet::OnInitDialog();
	
	if (GetWindowLong(GetSafeHwnd  (), GWL_STYLE) & WS_CHILD)
	{
...
//	ClipChildren(pDC);
	...
	return CBasePropertySheet::OnEraseBkgnd(pDC);
}
...
IMPL_USE_WND_HTMLHELP(CResizableSheet)

DWORD CResizableSheet::__GetDefaultHelpContextID () const
{
	CWnd *pParent = GetParentOwner();
	if(pParent)
	{
		int nQueryMsgResult = pParent->SendMessage(UM_QUERY_HELP_CONTEXT_FROM_PROP_SHEET, GetActiveIndex());
		if(nQueryMsgResult)
			return nQueryMsgResult;
		if(pParent->IsKindOf(RUNTIME_CLASS(CCWhiteBarDlg)))
			return ((CCWhiteBarDlg *)pParent)->m_nHelpId;
	}
	return 0;
}
