...
static std::list < CTitleTip * > g_pKbrdMsgWnd;

BEGIN_MESSAGE_MAP(CTitleTip, CToolTipCtrl)
...
LRESULT CALLBACK KeyboardHookProc( int code, WPARAM wParam, LPARAM lParam )
{
    if  ( code >= 0 )
    {
        for ( std::list < CTitleTip * >::iterator i = g_pKbrdMsgWnd.begin (); i != g_pKbrdMsgWnd.end (); ++i )
        {
            ( *i ) -> Hide ();
        }
    }
...
CTitleTip::~CTitleTip ()
{
    g_pKbrdMsgWnd.remove ( this );
    
    if  ( g_pKbrdMsgWnd.empty () && g_hKbrdMsgHook )
    {
...
//------------------------------------------------------------------------------
void CTitleTip::Show ( CRect rectTitle, LPCTSTR lpszTitleText, bool bShowInAnyCase )
{
    if  ( GetSafeHwnd () && rectTitle.IsRectEmpty () == FALSE )
    {
		...
		{
			TRACE("Mouse button down, tooltip do not show\n");
			return;
		}
        ...
        {
			TRACE1("Tooltip do not show? m_nMouseMoveDelay=%d\n", m_nMouseMoveDelay);
            --m_nMouseMoveDelay;
            return;
        }
		...
		m_rectTitle.InflateRect(1, 1, 1, 1);
//
//        m_rectTitle.top = -1;
//        m_rectTitle.left = -1;
...
//        m_rectTitle.bottom = rectTitle.Height () + 1;
	    ...
	    if  ( !m_bOnShow )
	        dc.SelectObject( m_pParentWnd -> GetFont () );

        CSize size = dc.GetTextExtent ( lpszTitleText );
        CSize offset = dc.GetTextExtent( _T(" ") );

        // Do not display if the text fits within available space
        if  ( bShowInAnyCase == false )
        {
            if  ( rectTitle.Width () >= size.cx )
            {
                return;
            }
        }
        rectTitle.right = rectTitle.left + size.cx + offset.cx;
        rectTitle.bottom = rectTitle.top + size.cy;
        
        TOOLINFO ti;
        ti.cbSize = sizeof ( TOOLINFO );
        ti.uFlags = m_bAbsPosFlag?TTF_TRACK | TTF_ABSOLUTE:TTF_SUBCLASS;
        ti.hwnd   = m_pParentWnd -> GetSafeHwnd ();    
        ti.uId    = 1;
        ti.hinst  = 0;
        ti.lpszText  = LPTSTR ( lpszTitleText );
        ti.rect = rectTitle;
        ...
        else
		{
			...
			if  ( m_bWrapLongText )
			{
				...
				SendMessage ( TTM_SETMAXTIPWIDTH, 0, ( LPARAM ) ( iScreenWidth - rectTitle.left ) );
			}
		}
        

		SendMessage ( TTM_TRACKPOSITION, 0, ( LPARAM ) MAKELPARAM ( rectTitle.left, rectTitle.top ) );
...
void CTitleTip::Hide ( )
{
    if  ( GetSafeHwnd () && m_bOnShow )
    {
        ...
        m_nMouseMoveDelay = m_nDelayConst;
...
void CTitleTip::OnMouseMove ( UINT nFlags, CPoint rcPoint )
{
    ...
    CToolTipCtrl::OnMouseMove ( nFlags, rcPoint );
}
