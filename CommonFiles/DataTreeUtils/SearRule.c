...
DTUT_PROC HDATA DTUT_PARAM DTUT_SearchForObjectRule( HDATA hRuleTreeRoot, HDATA hObjectData, CompareTwoObjects pCompareObjectFunc ) {
    ...
    _RPT0( _CRT_ASSERT, _T("DTUT_SearchForObjectRule Wrong parameters") );
		...
		if ( !pCompareObjectFunc )
			pCompareObjectFunc = DTUT_CompareTwoAvpObjects;
...
DTUT_PROC HDATA DTUT_PARAM DTUT_SearchForObjectRuleEx( HDATA hRuleTreeRoot, 
    ...
    _RPT0( _CRT_ASSERT, _T("DTUT_SearchForObjectRuleEx Wrong parameters") );
		...
		if ( pdwResult && !*pdwResult ) {
			...
			if ( !pCompareObjectFunc )
				pCompareObjectFunc = DTUT_CompareTwoAvpObjectsStr;
				...
				if ( hRuleRoot ) {
					...
					if ( nFirstObjectType != NS_OBJECT_TYPE_MYCOMPUTER ) {
						if ( nFirstObjectType != NS_OBJECT_TYPE_NWROOT )
							pRuleObjectName  = DTUT_GetPropValueAsString( DATA_Find_Prop(hCurrData, NULL, 0), NULL );
						else
							pRuleObjectName  = _tcscpy( (TCHAR *)DTUAllocator(SIZEOF_TCHAR * (DTU_TCSCLEN(NWNS_ROOTNAME) + 1)), NWNS_ROOTNAME );
					}
					else
						pRuleObjectName  = _tcscpy( (TCHAR *)DTUAllocator(SIZEOF_TCHAR), _T("") );
