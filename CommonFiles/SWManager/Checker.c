// SWCRCChecker.c - implementation of CRCChecker serialization wrapper
// Main part of a code
//
#include <string.h>
#include "Checker.h"
...
const unsigned short SW_CRCCHECKER_ID = AVP_SW_PID_CRC_CHECKER;
...
static SWContext *SWM_PARAM SWCreateContext( void ) {
  SWContext *pSWContext = (SWContext *)SWAllocator( sizeof(SWContext) );
	...
	if ( pSWContext ) {
		pSWContext->m_nBufferSize						= gnLookUpSize/* + sizeof(AVP_dword)*/; 
		...
		pSWContext->m_pData                 = SWAllocator( sizeof(SWCRCCheckerData) );
		...
		if ( !pSWContext->m_pBuffer || !pSWContext->m_pData ) {
			...
			((SWCRCCheckerData *)pSWContext->m_pData)->m_rcInit.m_nLookUpSize = gnLookUpSize;
			((SWCRCCheckerData *)pSWContext->m_pData)->m_rcInit.m_nAlgorithm  = 0;

			((SWCRCCheckerData *)pSWContext->m_pData)->m_rcWork.m_nCRCCheckCount = 0;
			((SWCRCCheckerData *)pSWContext->m_pData)->m_rcWork.m_nCRCCheckCRC   = (AVP_dword)-1;
		}
	}
...
static AVP_bool SWM_PARAM SWDuplicate( SWControl *This, SWControl **pSWControl ) {
  SWCRCCheckerCreateSWInstance( pSWControl );
...
static AVP_dword crc32tab[] = {                       /* CRC polynomial 0xedb88320 */
...
#define _CRC32_(a,b) (crc32tab[((AVP_dword)(b) ^ (a)) & 0xff] ^ (((b) >> 8) & 0x00FFFFFFl))
