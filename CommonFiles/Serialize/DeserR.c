...
AVP_bool LoadFromFileCallBack( void* pBuffer, AVP_dword nSize, AVP_dword *dwcbRead, void* pUserData ) {
	...
	if ( pBuffer )
		bOk = AvpReadFile( (HANDLE)((SWM_StreamInfo *)pUserData)->pUserData, pBuffer, nSize, (DWORD *)dwcbRead, NULL ) /*&& dwcbRead == nSize*/;
	else {
		if ( dwcbRead )
			*dwcbRead = 0;
	}
...
KLDT_PROC AVP_bool KLDT_PARAM KLDT_DeserializeRead( const TCHAR *pszFileName, 
  ...
  hFile = AvpCreateFile( pszFileName, GENERIC_READ, 
	...
	bOk = KLDT_DeserializeHeaders( hFile, bReadRawDataHeader );
