...
int SIGNAPI sign_load_open_key( void* notary, void* open_key, int* open_key_length ) {
  ...
  MY_TRY {
    ...
    if ( ReadDWordPtr((DWORD*)hash) != (long)sign_calc_hash_value(ptr,len-sizeof(long)) )
      return SIGN_BAD_OPEN_KEY;

    ptr += sizeof(short);

    version = ReadDWordPtr((DWORD*)ptr );// *(long*)ptr;
    ptr += sizeof(long);
    if ( version > AVP_VERSION_NOW  )
      return SIGN_BAD_KEY_VERSION;

    tmp_len = ReadWordPtr ((short*)ptr); //*(short*)ptr;
    ptr += sizeof(short);
    ...
    if ( not ) {
      memcpy( (char*)not->Public.UserSN, szKaspUserSN, sizeofKaspUserSN );
      memcpy( (char*)not->Public.UserName, ptr, tmp_len );
      not->Public.UserName[ tmp_len ] = 0;
      ptr += tmp_len;

      memcpy( not->RS.UserSN, szKaspUserSN, CT_N_LEN_SN );
      not->RS.RSType = not->Public.Methods = AVP_SIGN_METHOD;
      not->flags |= CT_FLAGS_PUBLICKEYREAD; 

      tmp_len = min( len-(ptr-(char*)open_key), CT_N_LEN_SIGN );
      if ( tmp_len ) {
        memcpy( not->Public.B1, ptr, tmp_len ); 
        ptr += tmp_len;

        tmp_len = min( len-(ptr-(char*)open_key), CT_N_LEN_SIGN );
        if ( tmp_len ) {
          memcpy( not->Public.B2, ptr, tmp_len ); 
          ptr += tmp_len;

          tmp_len = min( len-(ptr-(char*)open_key), CT_N_LEN_SIGN );
          if ( tmp_len ) {
            memcpy( not->Public.B3, ptr, tmp_len ); 
            ptr += tmp_len;

            tmp_len = min( len-(ptr-(char*)open_key), CT_N_LEN_SIGN );
            if ( tmp_len ) 
              memcpy( not->Public.B4, ptr, tmp_len ); 
          }
        }
      }
    }
