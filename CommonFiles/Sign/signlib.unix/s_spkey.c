...
int sign_save_pub_key( CTN_Signature* sign, void* open_key, short length, short* out_len, int text ) {
  ...
  memcpy( ptr, &version, sizeof(unsigned long) );       ptr += sizeof(unsigned long);
  memcpy( ptr, &name_len, sizeof(short) );              ptr += sizeof(short);
  memcpy( ptr, (char*)not.Public.UserName, name_len );  ptr += name_len;

  memcpy( ptr, not.Public.B1, CT_N_LEN_SIGN );   ptr += CT_N_LEN_SIGN;
  memcpy( ptr, not.Public.B2, CT_N_LEN_SIGN );   ptr += CT_N_LEN_SIGN;
  memcpy( ptr, not.Public.B3, CT_N_LEN_SIGN );   ptr += CT_N_LEN_SIGN;
  memcpy( ptr, not.Public.B4, CT_N_LEN_SIGN );   ptr += CT_N_LEN_SIGN;

  len = ((char *) ptr) - ((char*) hash_ptr);
