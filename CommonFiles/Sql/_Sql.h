...
public:
	...
	BOOL	GetDiagRec( BOOL i_bStatementHandle, int i_nRecNmb, LPTSTR o_strState, int& o_nNativeError, LPTSTR o_strMsg, int i_nMsgBufLen );
...
inline void CSql::GetErrorString( TCHAR* pBuf, DWORD BufLength ) {
	TCHAR	strState[6];
	int		nNativeError;
	GetDiagRec(TRUE, 1, strState, nNativeError, pBuf, BufLength);
}


inline void CSql::GetStateString( TCHAR* pBuf ) {
	int		nNativeError;
	GetDiagRec(TRUE, 1, pBuf, nNativeError, NULL, 0);
}


//-------------------------------------------------------------------------//
inline BOOL CSql::Trusted_Connect( TCHAR* i_strDSN, TCHAR* AppName, HWND hWnd, BOOL i_bNoPrompt ) {
	TCHAR szConnStrIn[255];

	wsprintf( szConnStrIn, _T("DSN=%s; Trusted_Connection=yes;APP=%s;"), i_strDSN, AppName );
	if ( m_bDisableAutoTranslation )
		lstrcat(szConnStrIn, _T("AutoTranslate=no;"));

	return ConnectInternal(szConnStrIn, hWnd, i_bNoPrompt);
}


//-------------------------------------------------------------------------//
inline BOOL CSql::Trusted_Connect2( TCHAR* i_strServerName, TCHAR* i_strDatabase, TCHAR* AppName, HWND hWnd, BOOL i_bNoPrompt ) {
	TCHAR szConnStrIn[255];
	const TCHAR* at;
	if ( m_bDisableAutoTranslation )
		at = _T("no");
		...
		at = _T("yes");
	wsprintf( szConnStrIn, _T("DRIVER={SQL Server};SERVER=%s;DATABASE=%s;Trusted_Connection=yes;APP=%s;AutoTranslate=%s;"), i_strServerName, i_strDatabase, AppName, at );
	return ConnectInternal( szConnStrIn, hWnd, i_bNoPrompt );
}
