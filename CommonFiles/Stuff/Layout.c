...
int Layout_GetOtherUnknownMetric (int nUnknownMetric) {
   ...
   // Check assumptions.
   adgASSERT(ISMETRIC(nUnknownMetric));
...
CHILD* Layout_FindChild (CHILD* pChildList, int idcChild) {
   ...
   adgASSERT(adgINRANGE(0, idcChild, IDC_LASTCHILD));
   ...
   adgFAIL(__TEXT("Child not found in child list"));
   return(NULL);
}
...
CHILD* Layout_CreateChildList (HWND hwndParent, int* pnChildren) {
   ...
   pChildList = malloc((*pnChildren + 2) * sizeof(CHILD));
   adgASSERT(pChildList);
   if (!pChildList)
      return(NULL);
   pChild = pChildList;
   ...
   switch (pRule->Action) {
      ...
      case lHCENTER:              // Horizontally center a control/group
         ...
         for (nRules = 0, pr = pRules; pr->Action != lEND; pr++)
            nRules++;
         nRules++;
         prNew = _alloca(nRules * sizeof(RULE));
         prn = prNew;
         ...
         for (pChild = pChildList; pChild->idc != IDC_LASTCHILD; pChild++)
            nChildren++;
         nChildren++;
         pChildListNew = _alloca(nChildren * sizeof(CHILD));
         ...
         if (!Layout_ApplyRules (hwndParent, prNew, pChildListNew)) {
            adgFAIL(__TEXT("Unable to apply rules to centered children"));
            return(FALSE);
         }
...
BOOL Layout_ComputeLayout (HWND hwndParent, RULE* pRules) {
   ...
   if (!Layout_ApplyRules(hwndParent, pRules, pChildList)) {
      adgFAIL(__TEXT("Unable to apply rules"));
      return(FALSE);
   }
   ...
   for (pChild = pChildList + 1; pChild->idc != IDC_LASTCHILD; pChild++) {
      ...
      if (!pChild->fFixed) {
         HWND hwndChild = GetDlgItem(hwndParent, pChild->idc);
				 // 20.09.00 Victor
				 if ( pChild->anMetric[lpWIDTH] < 0 )
					 pChild->anMetric[lpWIDTH] = 0;
				 if ( pChild->anMetric[lpHEIGHT] < 0 )
					 pChild->anMetric[lpHEIGHT] = 0;
