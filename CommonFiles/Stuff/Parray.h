...
#else
  template<typename U> friend void destroy_array( CPArray<U>& arr );
	template<typename U> friend void destroy_rest_array( CPArray<U>& arr, uint size );
  template<typename U> friend void join_arrays( CPArray<U>& to, CPArray<Type>& from );
  template<typename U> friend void insert_array( CPArray<U>& to, CPArray<U>& from, uint index );
  template<typename U> friend uint find_in_array( const CPArray<U>& arr, const U* el );
#endif
};
...
#else
  template<typename U> friend void  for_each_in_array( const CPIArray<U>& arr, void (*func)(U*) );
  template<typename U> friend void  for_each_in_array( const CPIArray<U>& arr, void (*func)(U*, void*), void* pars );
  template<typename U> friend uint  first_that_in_array( const CPIArray<U>& arr, bool (*func)(U*, void*), void* pars, uint from );
#endif
};
...
#else
  template<typename U> friend void  for_each_in_array( const CPMIArray<U>& arr, void (U::* func)() );
  template<typename U> friend void  for_each_in_array( const CPMIArray<U>& arr, void (U::* func)(void*), void* pars );
  template<typename U> friend uint  first_that_in_array( const CPMIArray<U>& arr, bool (U::* func)(void*), void* pars, uint from );
#endif
};
...
inline Type* CPArray<Type>::RemoveInd( uint delIndex, DelType del ) {
	if ( count ) {
		...
		//VIK 23.06.03
		if ( --count )
			memcpy( d, d+1, (count - delIndex)*sizeof(TPtr) );
		return r;
	}
	return 0;
}
...
void destroy_array( CPArray<Type>& arr ) {
  uint i = 0;
  for( CPArray<Type>::TPtr* parr=arr.parr; i<arr.count; i++,parr++ ) {
    delete *parr;
    *parr = 0;
  }  
  arr.count = 0;
...
void destroy_rest_array( CPArray<Type>& arr, uint size ) {
  uint i = size;
  for( CPArray<Type>::TPtr* parr=arr.parr + size; i<arr.count; i++,parr++ ) {
    delete *parr;
    *parr = 0;
  }  
  arr.count = size;
...
void join_arrays( CPArray<Type>& to, CPArray<Type>& from ) {
  ...
  if ( newUpper > to.upper ) {
    CPArray<Type>::TPtr* p_tmp = (CPArray<Type>::TPtr*)memcpy( new CPArray<Type>::TPtr[ to.upper = newUpper ], to.parr, to.count * sizeof(CPArray<Type>::TPtr) );
    delete [] to.parr;
    to.parr = p_tmp;
  }
  memcpy( to.parr+to.count, from.parr, from.count * sizeof(CPArray<Type>::TPtr) );
  to.count = newCount;
...
void insert_array( CPArray<Type>& to, CPArray<Type>& from, uint index ) {
  ...
  for( newUpper=to.upper; newUpper < newCount; newUpper+=to.delta );
  CPArray<Type>::TPtr* p_tmp = new CPArray<Type>::TPtr[ to.upper = newUpper ];
  memcpy( p_tmp,                  to.parr,       index * sizeof(CPArray<Type>::TPtr) );
  memcpy( p_tmp+index,            from.parr,     from.count * sizeof(CPArray<Type>::TPtr) );
  memcpy( p_tmp+index+from.count, to.parr+index, (to.count-index) * sizeof(CPArray<Type>::TPtr) );
...
uint find_in_array( const CPArray<Type>& arr, const Type* el ) {
  CPArray<Type>::TPtr* parr = arr.parr;
...
uint first_that_in_array( const CPIArray<Type>& arr, bool ( *func ) ( Type*,void* ), void* pars, uint from ) {
  CPArray<Type>::TPtr* parr = arr.parr+from;
