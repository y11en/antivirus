...
static int put_bit (CFile& f, BYTE bit)
{
  *(WORD *)outbuf >>= 1;
...
DWORD squeeze ( CFile& inf, CFile& outf, DWORD length, BYTE xor_byte )
{
 int     readed_size,i;
...
*/
  ...
  readed_size =inf.Read(buffer,readed_size);
...
//	  memcpy (buffer+buffer_top,inf+icount,readed_size);
	  readed_size =inf.Read(buffer+buffer_top,readed_size);
	  icount+=readed_size;
