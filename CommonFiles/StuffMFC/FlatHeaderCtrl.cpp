...
INT CFlatHeaderCtrl::DrawText(CDC* pDC, CRect rect, LPHDITEM lphdi)
{
	...
	if(rect.Width()>0 && lphdi->mask&HDI_TEXT && lphdi->fmt&HDF_STRING)
	{
		...
		if (m_bSelectFirstColumn)
		{
			CFont fBold;
			fBold.CreateFont(m_nTextSize, 0, 0, 0, m_bTextBold?FW_BOLD:0, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, PROOF_QUALITY, VARIABLE_PITCH, "MS Sans Serif");
			pfOld = pDC->SelectObject(&fBold);
		}
		...
		if (pfOld) pDC->SelectObject(pfOld);
	}
