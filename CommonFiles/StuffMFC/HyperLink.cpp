...
HINSTANCE CHyperLink::GotoURL(LPCTSTR url, int showcmd)
{
	...
	CString sURL = url;
	// VM 1/10/99 - Given "url" could be in different case - so make it in lower case
	sURL.MakeLower();

	if ((sURL.Find(L'@') != -1) && (sURL.Find(_T("mailto:")) == -1))	sURL = _T("mailto:") + sURL;

    // First try ShellExecute()
    HINSTANCE result = ShellExecute(NULL, _T("open"), sURL, NULL,NULL, showcmd);
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key) == ERROR_SUCCESS) {
            ...
            if (GetRegKey(HKEY_CLASSES_ROOT,key,key) == ERROR_SUCCESS) {
                ...
                //result = (HINSTANCE) WinExec(key,showcmd);
								...
								BOOL bResult = ::CreateProcess( key, NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &piProcess );
								
								if ( bResult )
									result = HINSTANCE(HINSTANCE_ERROR + 1);
								else
									result = HINSTANCE(::GetLastError());

								if ( piProcess.hProcess ) 
									::CloseHandle( piProcess.hProcess );
								if ( piProcess.hThread ) 
...
void CHyperLink::DestroyGlobalVars()
{
	if (g_hLinkCursor) DestroyCursor(g_hLinkCursor);
	g_hLinkCursor = NULL;
}
