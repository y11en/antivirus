...
{
	hicr=GetSysColor(COLOR_BTNHIGHLIGHT);		//get the button base colors
	midcr=GetSysColor(COLOR_BTNFACE);
	locr=GetSysColor(COLOR_BTNSHADOW);
}
...
void CShadeButtonST::SetShadeColor(COLORREF crhicr, COLORREF crmidcr, COLORREF crlocr)
{
	hicr=crhicr;		//get the button base colors
	midcr=crmidcr;
	locr=crlocr;
}
