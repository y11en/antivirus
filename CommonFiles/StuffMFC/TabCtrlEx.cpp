...
{
	m_crSelColour = ::GetSysColor(COLOR_BTNTEXT);
	m_crUnselColour = ::GetSysColor(COLOR_HIGHLIGHTTEXT);
	m_nBg = ::GetSysColor(COLOR_3DFACE);
}
...
void CTabCtrlEx::SetBgColor(COLORREF color)
{
	m_nBg = color;
	Invalidate();
}
