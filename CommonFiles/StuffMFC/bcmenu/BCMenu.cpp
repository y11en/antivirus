...
#include <debugging.h>       //SK: makes A2W and other spiffy AFX macros work
...
void BCMenu::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	AB_TRY
	{
		...
		if (state & MF_SEPARATOR)
		{
			...
			rect.left += m_iconX + 4 + GAP;//A
			//pDC->FillRect(rect, &CBrush(GetSysColor(COLOR_3DHIGHLIGHT)));//A
			pDC->FillRect(rect, &CBrush(RGB(245, 245, 245)));
		...
		{
			CRect rect2;
			BOOL standardflag = FALSE, selectedflag = FALSE, disableflag = FALSE, checkflag = FALSE;
			...
			if (lpDIS->itemID == m_iDefaultItem) m_lf.lfWeight = FW_BOLD;
			...
			if(lpDIS->itemData != NULL)
			{
				...
				if (state & ODS_CHECKED && nIconNormal < 0)
				{
					if (state & ODS_SELECTED && m_selectcheck > 0) 
						...
						if (m_unselectcheck > 0) checkflag = TRUE;
				}
				else
				{
					if (nIconNormal != -1)
				...
				if (state & ODS_GRAYED) disableflag = TRUE;
			}
			else
			{
			  strText.Empty();
			...
			if (state & ODS_SELECTED)	 // draw the down edges
			{
				if (!disableflag)
				{
					...
					//A if (checkflag || standardflag || selectedflag || disableflag || state & ODS_CHECKED)
					rect2.SetRect(rect.left + m_iconX + 4 + GAP, rect.top, rect.right, rect.bottom);

					pDC->FillRect(rect2, &m_brSelect);
					//pDC->FrameRect(rect2, &CBrush(GetSysColor(COLOR_3DHILIGHT)));//A
			...
			{
				CPen *pOldPen = pDC->SelectObject (&m_penBack);
	//A			pDC->FillRect(rect, &m_brBackground);

				pDC->FillRect(rect, &CBrush(RGB(245, 245, 245)));//A
				
				CRect rPic = rect;//A
				rPic.right = rPic.left + m_iconX + 4 + GAP;//A
				pDC->FillRect(rPic, &m_brBackground);//A

				pDC->SelectObject (pOldPen);

				// draw the up edges
	//A			pDC->Draw3dRect(rect,m_clrBack,m_clrBack);
			}
			...
			if (checkflag || standardflag || selectedflag || disableflag)
			{
				...
				if (checkflag && checkmaps)
				{
					...
					pDC->Draw3dRect(rect2, m_clrBack, m_clrBack);
					checkmaps->Draw(pDC, (state & ODS_SELECTED)?1:0, CPoint(rect.left + 2, rect.top + 2 + dy), ILD_TRANSPARENT);
				}
				else
				{
					if (disableflag)
					{
						if (!selectedflag)
						{
							...
							if (GetBitmapFromImageList(pDC, bitmap, xoffset, bitmapstandard))
							{
								rect2.SetRect(rect.left, rect.top + dy, rect.left + m_iconX + 4, rect.top + m_iconY + 4 + dy);
								pDC->Draw3dRect(rect2, m_clrBack, m_clrBack);

								if (disable_old_style)
										DitherBlt(lpDIS->hDC, rect.left + 2, rect.top + 2 + dy, m_iconX, m_iconY, (HBITMAP)(bitmapstandard), 0, 0);
									else
										DitherBlt2(pDC, rect.left + 2,rect.top + 2 + dy, m_iconX, m_iconY, bitmapstandard, 0, 0);
...
//A					else
//A					{
//A						// And the standard Grey text:
//A						pDC->SetTextColor(m_clrBack);
//A						pDC->DrawText(leftStr,rectt, nFormat);
//A						if (tablocr!=-1) pDC->DrawText (rightStr,rectt,nFormatr);
//A					}
				}
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	try
	{
		...
		{
			lpMIS->itemWidth = 0;
//A			lpMIS->itemHeight = GetSystemMetrics(SM_CYMENU) >> 1;
			lpMIS->itemHeight = GetSystemMetrics(SM_CYMENU) / 3 - 1;
		}
		else
		{
			CFont m_fontMenu;
			...
			else		//it's Win32suckx
			{
				...
				size.cy = DrawText(pDC->m_hDC, (LPCTSTR)lpstrText, wcslen(lpstrText), &rect, DT_SINGLELINE|DT_LEFT|DT_VCENTER|DT_CALCRECT);
			...
			lpMIS->itemWidth = m_iconX + t.cx + m_iconX + GAP + 30;
...
BOOL BCMenu::LoadMenu(HINSTANCE hInst, int nResource)
{
  return(BCMenu::LoadMenu(hInst, MAKEINTRESOURCE(nResource)));
};

BOOL BCMenu::LoadMenu(HINSTANCE hInst, LPCTSTR lpszResourceName)
{
  TRACE(_T(
    "IMPORTANT:Use BCMenu::DestroyMenu to destroy Loaded Menu's\n"));
  ASSERT_VALID(this);
  ...
  HINSTANCE m_hInst = hInst;
  // Find the Menu Resource:
  if (!m_hInst) m_hInst = AfxFindResourceHandle(lpszResourceName,RT_MENU);
...
void BCMenu::SetMenuDefaultItem(UINT iCmd)
{
	m_iDefaultItem = iCmd;
	::SetMenuDefaultItem(m_hMenu, iCmd, 0);
}
