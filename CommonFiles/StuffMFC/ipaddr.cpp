...
void DDX_IPAddr(CDataExchange* pDX, int nIDC, CString& szAddr)
{
	...
	if (!pDX->m_bSaveAndValidate)
	{
		CString copy_szAddr(szAddr) ; //bug fix
	   ...
	   // Get numbers
		...
		if (ii < 3)
		{
		    int nIndex = copy_szAddr.Find(_T('.')); // bug fix
		    if (-1 != nIndex)
		    {
			nAddr[ii] = (unsigned char) _ttoi(copy_szAddr.Left(nIndex));//bug fix
			copy_szAddr = copy_szAddr.Mid(nIndex + 1); // bug fix
		    }
		}
		else
		    nAddr[ii] = (unsigned char) _ttoi(copy_szAddr);//bug fix
	    }
