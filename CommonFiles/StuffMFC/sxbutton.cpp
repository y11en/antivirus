...
{	
	...
	if( !bDRAWFOCUSONLY )
	{
		...
		if (m_bFlashMode) 
		{
			CRect R = rectControl;
			R.DeflateRect(1, 1);
			pDC->FillSolidRect(&R, GetSysColor(COLOR_ACTIVECAPTION));
...
void CSXButton::Flash(DWORD dwTimes, DWORD dwTimeout)
{
	m_nFlashCurCount = 0;
	m_nFlashCount = dwTimes * 2;
	m_nTimer = SetTimer(TID_FLASHING, dwTimeout, NULL);
}

void CSXButton::OnTimer(UINT nIDEvent)
{
	m_bFlashMode = (m_nFlashCurCount & 1)?FALSE:TRUE;
	Redraw();
	CButton::OnTimer(nIDEvent);
	m_nFlashCurCount++;
	if (m_nFlashCurCount >= m_nFlashCount) KillTimer(m_nTimer);
}

void CSXButton::SetFlat(BOOL bFlat)
{
	m_bFlat = bFlat;
}