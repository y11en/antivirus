...
{
    while(count--)
        seed = CRC32Tbl[(unsigned char)(seed) ^ *data++] ^ (seed >> 8);
...
bool calculateFileCRC(const KLUPD::Path &sourceFileName, unsigned int &crc)
{
    ...
    KLUPD::LocalFile file(sourceFileName);
    std::vector<unsigned char> target;
    if(!KLUPD::isSuccess(file.read(target)))
        return false;
    crc = CRC32(target.empty() ? 0 : &target[0], target.size(), 0);
    return true;
}
