...
bool __stdcall memio_FileSetSize(tFile* pFile, tFileSize nSize)
{
	...
	if (pmemio->pos > nSize)
		pmemio->pos = nSize;
	memio_FileWrite(pFile, (unsigned char*)&pFile, 0, NULL); // dummy write for memiry allocation check
	return true;
}
...
bool __stdcall prague_FileSeek(tFile* pFile, tFilePos nPos, int nSeekMethod, tFilePos* pnNewPos)
{
	...
	case SEEK_CUR:
		...
		break;
	case SEEK_END:
        if (!prague_FileGetSize(pFile, &size))
            return false;
		nPos += size;
		break;
	case SEEK_SET:
...
{
    bool bRes = PR_SUCC(CALL_IO_SetSize((hIO)pFile->hHandle, nSize));
    if (bRes && (tFileSize)pFile->nPos > nSize)
        pFile->nPos = nSize;
    return bRes;
}
