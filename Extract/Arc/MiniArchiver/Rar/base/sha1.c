...
// ----------------------------------------------------------------------------
  ...
  #if defined(_M_IX86) || defined(_M_I86) || defined(__alpha) || defined(_M_X64)
...
tVOID SHA1_Initial(tSHA1_CTX* context)
{
...
tVOID SHA1_Process(tSHA1_CTX* context, tBYTE* data, tUINT len)
{
unsigned int i, j;
...
tVOID SHA1_Final(tSHA1_CTX* context, tUINT digest[5])
{
    uint i, j;
    ...
    ch='\200';
    SHA1_Process(context, &ch, 1);
    while ((context->count[0] & 504) != 448) {
        ch=0;
        SHA1_Process(context, &ch, 1);
    }
    SHA1_Process(context, finalcount, 8);  /* Should cause a SHA1Transform() */
    for (i = 0; i < 5; i++) {
