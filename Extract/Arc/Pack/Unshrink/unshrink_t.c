...
// -------- Monday,  09 December 2002,  14:26 --------
// -------------------------------------------
// Copyright (c) Kaspersky Labs. 1996-2002.
// -------------------------------------------
// Project     -- Not defined
// Sub project -- Not defined
// Purpose     -- UnShrink decompressor
// Author      -- Andy Nikishin
// File Name   -- unshrink_t.c
// Additional info
//    UnShrink decompressor. Uses fro ZIP
// -------------------------------------------
...
typedef struct tag_hi_Transformer 
{
	...
	UnshrinkData*           data; // pointer to the "Transformer" data structure
} *hi_Transformer;
...
mPROPERTY_TABLE(Transformer_PropTable)
	...
	mSHARED_PROPERTY( pgINTERFACE_VERSION, Transformer_VERSION )
	mSHARED_PROPERTY_PTR( pgINTERFACE_COMMENT, "Unshrink extractor", 19 )
	mLOCAL_PROPERTY_BUF( pgTRANSFORM_SIZE, UnshrinkData, qwSizeOfFile, cPROP_BUFFER_WRITE )
	mLOCAL_PROPERTY_BUF( pgINPUT_SEQ_IO, UnshrinkData, hInputIO, cPROP_BUFFER_READ_WRITE )
	mLOCAL_PROPERTY_BUF( pgOUTPUT_SEQ_IO, UnshrinkData, hOutputIO, cPROP_BUFFER_READ_WRITE )
...
tERROR pr_call Transformer_Register( hROOT root ) 
{
	...
	error = CALL_Root_RegisterIFace(
		...
		PID_UNSHRINK,                           // plugin identifier
		...
		sizeof(UnshrinkData),                   // memory size
		0                                       // interface flags
	);
...
tERROR error ;
	...
	error = CALL_SYS_ObjHeapAlloc(_this, (tPTR*)&_this->data->area, sizeof(union work));
...
tDWORD ret_val;
	...
	ret_val = (tDWORD)unShrink(_this->data);
	if(ret_val == 0)
    {
        if (_this->data->zip_corrupt)
