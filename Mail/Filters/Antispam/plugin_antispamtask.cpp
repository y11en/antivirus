...
extern tERROR pr_call AntispamFilter_Register( hROOT root );
...
tBOOL __stdcall DllMain( tPTR hInstance, tDWORD dwReason, tERROR* pERROR ) {
	...
	switch( dwReason ) {
		...
		case PRAGUE_PLUGIN_LOAD :
			...
			*pERROR = errOK;
			//resolve  my imports
			//if ( PR_FAIL(*pERROR=CALL_Root_ResolveImportTable(root,&count,import_table_variable...,PID_ANTISPAMTASK)) ) {
			//   PR_TRACE(( root, prtERROR, "cannot resolve import table for ..." ));
			//   return cFALSE;
			//}

			//register my exports
			//CALL_Root_RegisterExportTable( root, &count, export_table_..., PID_ANTISPAMTASK );
			...
			if ( PR_FAIL(*pERROR=AntispamFilter_Register(root)) ) {
				PR_TRACE(( root, prtERROR, "cannot register \"AntispamFilter\" interface"));
				return cFALSE;
			}
