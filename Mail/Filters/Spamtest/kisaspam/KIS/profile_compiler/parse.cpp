...
int compiler_warning(const char *format, ...)
{
	...
	TCHAR buf[1024]; 
	buf[sizeof(buf)-1] = 0;
	snprintf(buf, sizeof(buf)-2, "%s", "Warning: ");
	vsnprintf(buf+strlen(buf), sizeof(buf)-2 - strlen(buf), format, args);
...
int compiler_error(const char *format, ...)
{
	...
	TCHAR buf[1024]; 
	buf[sizeof(buf)-1] = 0;
	snprintf(buf, sizeof(buf)-2, "%s", "Error: ");
	vsnprintf(buf+strlen(buf), sizeof(buf)-2 - strlen(buf), format, args);
 ...
 * Creates new list
 */
list_id parse_email(const char *const_fname,
                    #ifndef PROFILE_COMPILER
...
#else
    res_xml = LoadXMLFile(const_fname, tag, tXMLProfile, &s, 0, CP_KOI8R);
#endif
 ...
 * Parsing of ip list.
 */
list_id parse_ip_list(const char *const_fname,
                    #ifndef PROFILE_COMPILER
                        XmlFilesPool *xmlPool, 
                    #endif // PROFILE_COMPILER
                      list_db *ldb)
 ...
 * Parsing of RBL list.
 */
list_id parse_rbl_list(const char *const_fname, 
                        #ifndef PROFILE_COMPILER
                            XmlFilesPool *xmlPool, 
                        #endif // PROFILE_COMPILER
                       list_db *ldb)
 ...
 */
profile *parse_profile(const char *const_fname, 
#ifndef PROFILE_COMPILER
                       XmlFilesPool *xmlPool, 
                       list_db *ldb,
                       content_filter *cf,
...
#define RETURN_NULL				\
	...
	if(profile_type == PROFILE_COMMON)
	    ...
	    {
#ifdef PROFILE_COMPILER
                compiler_error("profile %s: kis compiler couldnt have private profiles\n");
                RETURN_NULL;
#else
		if(profile_rcpts)
...
#define SKIP_RULE				\
		...
		conditions_iterator++)
		{
			...
			{
#ifdef PROFILE_COMPILER
                            if(compiler_warning("profile %s, rule %u: KIS can\'t use  SMTPFromList condition.\n", const_fname, rule_counter))
                                RETURN_NULL;
                            else
                                SKIP_RULE;
#else
			...
			{
#ifdef PROFILE_COMPILER
                            if(compiler_warning("profile %s, rule %u: KIS can\'t use SMTPToList condition.\n", const_fname, rule_counter))
                                RETURN_NULL;
                            else
                                SKIP_RULE;
#else
			...
			{
#ifdef PROFILE_COMPILER
                            if(compiler_warning("profile %s, rule %u: KIS can\'t use IPFromList condition.\n", const_fname, rule_counter))
                                RETURN_NULL;
                            else
                                SKIP_RULE;
#else
			...
			{
#ifdef PROFILE_COMPILER
                            if(compiler_warning("profile %s, rule %u: KIS can\'t use IPFromDNSBlackList condition.\n", const_fname, rule_counter))
                                RETURN_NULL;
                            else
                                SKIP_RULE;
#else
			...
			{
#ifdef PROFILE_COMPILER
                            if(compiler_warning("Profile %s, rule %u: KIS Can\'t use CategoryMatch condition.\n", const_fname, rule_counter))
                                RETURN_NULL;
                            else
                                SKIP_RULE;
#else
