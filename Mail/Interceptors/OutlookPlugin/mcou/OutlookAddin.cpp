...
// Global pointer to Extension class
IMCOUExchExt *g_pIMCOUExchExt = NULL;

CComModule _Module;

BEGIN_OBJECT_MAP(ObjectMap)
OBJECT_ENTRY(CLSID_Addin, CAddin)
OBJECT_ENTRY(CLSID_PageAntivirus, CPageAntivirus)
END_OBJECT_MAP()
...
STDAPI DllCanUnloadNow(void)
{
    ...
    if (PrxDllCanUnloadNow() != S_OK)
        return S_FALSE;
#endif
	ATLTRACE("DllCanUnloadNow return 0x%X\n", _Module.GetLockCount());
...
STDAPI DllRegisterServer(void)
{
	...
	HRESULT hRes = RegisterPlugin(_Module.GetModuleInstance());
    if(FAILED(hRes))
        return hRes;
