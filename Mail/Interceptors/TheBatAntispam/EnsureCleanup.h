...
public:
	...
	// Re-assign the object without cleaning it up
	TYPE Detach() { 
	  TYPE tt = TYPE(m_t);
	  m_t = tInvalid;
	  return tt;  
...
private:
	CEnsureCleanup(const CEnsureCleanup&);
	CEnsureCleanup& operator=(const CEnsureCleanup&);	// no copies
...
MakeCleanupClass(CEnsureDestroyMenu,		HMENU,		DestroyMenu);
MakeCleanupClassX(CEnsureFindClose,			HANDLE,		FindClose,		INVALID_HANDLE_VALUE);
MakeCleanupClass(CEnsureFreeSid,			PSID,		FreeSid);
MakeCleanupClass(CEnsureInternetClose,		HINTERNET,	InternetCloseHandle);
...
// Special class for releasing a semaphore.
// Special class is required because ReleaseSemaphore requires 3 parameters
class CEnsureReleaseSemaphore {
public:
   CEnsureReleaseSemaphore(HANDLE hSemaphore = NULL) : m_hSemaphore(hSemaphore) { }
   ~CEnsureReleaseSemaphore() { Cleanup(); }

   HANDLE operator=(HANDLE hSemaphore) { 
      Cleanup(); 
      m_hSemaphore = hSemaphore; 
      return(m_hSemaphore); 
   }
   operator HANDLE() { return(m_hSemaphore); }
...
// special class for auto-setting an event
class CEnsureSetEvent
{
public:
	CEnsureSetEvent(HANDLE hEvent) : m_hEvent(hEvent) 
	{
		ResetEvent(m_hEvent);
	}
...
private:
	...
	CEnsureSetEvent(const CEnsureSetEvent&);
	CEnsureSetEvent& operator=(const CEnsureSetEvent&);
};
