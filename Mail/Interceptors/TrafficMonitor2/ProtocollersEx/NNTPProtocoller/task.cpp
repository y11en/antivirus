...
struct pr_novtable CNNTPProtocoller : public cTask {
private:
...
public:
	mDECLARE_INITIALIZATION(CNNTPProtocoller)
};
// AVP Prague stamp end
...
// Parameters are:
tERROR CNNTPProtocoller::ObjectInitDone() {
	tERROR error = errOK;
	PR_TRACE_FUNC_FRAME( "Task::ObjectInitDone method" );

	// Place your code here
	if ( PR_SUCC(error) )
		error = PrepareObject<CNNTPProtocoller>(this, m_dwSessionCount, &m_hSessionCS, m_traffic_monitor, m_tm);
...
// Parameters are:
tERROR CNNTPProtocoller::ObjectPreClose() {
	tERROR error = errOK;
	PR_TRACE_FUNC_FRAME( "Task::ObjectPreClose method" );

	// Place your code here
	error = ReleaseObject<CNNTPProtocoller>(this, &m_hSessionCS, m_traffic_monitor, m_tm);
...
tERROR CNNTPProtocoller::MsgReceive( tDWORD p_msg_cls_id, tDWORD p_msg_id, hOBJECT p_send_point, hOBJECT p_ctx, hOBJECT p_receive_point, tVOID* p_par_buf, tDWORD* p_par_buf_len ) {
	...
	return ProtocollerMsgReceive<NntpProtocolAnalyzer>("nntp", *this, po_NNTP, p_msg_cls_id, p_msg_id, p_send_point, p_ctx, p_receive_point, p_par_buf, p_par_buf_len, m_dwSessionCount, m_hSessionCS, (tTaskState)m_state);
}
// AVP Prague stamp end
...
// Parameters are:
tERROR CNNTPProtocoller::SetSettings( const cSerializable* p_settings ) {
	tERROR error = errOK;
...
// Parameters are:
tERROR CNNTPProtocoller::GetSettings( cSerializable* p_settings ) {
	tERROR error = errOK;
...
// Parameters are:
tERROR CNNTPProtocoller::AskAction( tActionId p_actionId, cSerializable* p_actionInfo ) {
	tERROR error = errNOT_IMPLEMENTED;
...
// Parameters are:
tERROR CNNTPProtocoller::SetState( tTaskRequestState p_state ) {
	tERROR error = errOK;
	...
	case TASK_REQUEST_STOP:
		...
		PR_TRACE((this, prtIMPORTANT, "nntp\tProcessing stop request..."));
		m_state = TASK_STATE_COMPLETED;
		PR_TRACE((this, prtIMPORTANT, "nntp\tReleasing trafmon"));
		...
		PR_TRACE((this, prtIMPORTANT, "nntp\tSending pseudo stop"));
		if ( errOK_DECIDED == sysSendMsg(pmcTRAFFIC_PROTOCOLLER, pmSESSION_PSEUDO_STOP_REQUEST, 0,0,0))
		{
			...
			PR_TRACE((this, prtIMPORTANT, "nntp\tWaiting sessions (if any)"));
			...
			while ( cTRUE ) 
			{
				...
				{
					PR_TRACE((this, prtIMPORTANT, "nntp\tStill %d sessions to wait...", sess_count));
					last_trace = t;
				}
				Sleep(100);
			}
		}
		PR_TRACE((this, prtIMPORTANT, "nntp\tStop request processed"));
		break;
	default:
		m_state = TASK_STATE_COMPLETED;
		break;
...
// Parameters are:
tERROR CNNTPProtocoller::GetStatistics( cSerializable* p_statistics ) {
	tERROR error = errOK;
...
tERROR CNNTPProtocoller::Register( hROOT root ) {
	tERROR error;
// AVP Prague stamp end
...
mpPROPERTY_TABLE(Task_PropTable)
	...
	mpLOCAL_PROPERTY_BUF( pgTASK_TM, CNNTPProtocoller, m_tm, cPROP_BUFFER_READ_WRITE | cPROP_WRITABLE_ON_INIT | cPROP_REQUIRED )
...
// AVP Prague stamp begin( Registration call,  )
	...
	error = CALL_Root_RegisterIFace(
		...
		PID_NNTPPROTOCOLLER,                    // plugin identifier
		...
		sizeof(CNNTPProtocoller)-sizeof(cObjImpl),// memory size
		0                                       // interface flags
	);
...
tERROR pr_call Task_Register( hROOT root ) { return CNNTPProtocoller::Register(root); }
// AVP Prague stamp end
