...
#include <plugin/p_nntpprotocoller.h>
...
public:
	...
	__declspec(dllexport) void TestMailProcess_Disinfect (const string& data_folder);
	__declspec(dllexport) void TestMailProcess_Delete (const string& data_folder);
...
public:
	...
	hIO io_ToProto;
	DWORD mc_ToProto_flags;
	std::string mc_ToProto_virname;
