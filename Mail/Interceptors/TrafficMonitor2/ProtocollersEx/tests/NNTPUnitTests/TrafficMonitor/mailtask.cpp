...
/*
	...
	*/
		...
		error = sysRegisterMsgHandler(pmc_MAILCHECKER_PROCESS, rmhDECIDER, this, IID_ANY, PID_ANY, IID_ANY, PID_ANY );
...
tERROR CTrafficMonitorTask::MsgReceive( tDWORD p_msg_cls_id, tDWORD p_msg_id, hOBJECT p_send_point, hOBJECT p_ctx, hOBJECT p_receive_point, tVOID* p_par_buf, tDWORD* p_par_buf_len ) {
	...
	if (p_msg_cls_id == pmc_MAILTASK_PROCESS || p_msg_cls_id == pmc_MAILCHECKER_PROCESS)
	{
		...
		if (m_testManager->bTraceMailProcess)
		{
			...
			//поменяем на заранее заготовленный и проставим все поля
			mc_params->p_MailCheckerVerdicts = m_testManager->mc_ToProto_flags;
			mc_params->p_szVirusName = m_testManager->mc_ToProto_virname.c_str();
			if (mc_params->p_MailCheckerVerdicts & mcv_MESSAGE_DELETED)
			{
				(cIO*)p_ctx->sysCloseObject();
			}
			else aux::pump_io_to_io (m_testManager->io_ToProto, (cIO*)p_ctx);
