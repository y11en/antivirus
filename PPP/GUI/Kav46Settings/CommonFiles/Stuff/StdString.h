#if !defined(WIN32_LEAN_AND_MEAN) || !defined(SS_NO_CONVERSION) || !defined(SS_NO_BSTR)
#pragma message("Recommended to define SS_NO_CONVERSION and SS_NO_BSTR macro before including StdString.h (else very big headers will be included and compilation slows down)")
#endif


// Если появится новая версия этого файла ВНЕСИТЕ В НЕГО УКАЗАННЫЕ ИЗМЕНЕНИЯ:
// Сделаны небольшие изменения для того, чтобы не инклудились очень большие хедера (WTYPES.H COMPTR.H afxconv.h), для этого
// перед инклудом stdstring.h укажите следующие дефайны:
...
//
//	  2000-MAR-07 - Thanks to Ullrich Pollдhne for catching a range bug in one
...
// Turn off unavoidable compiler warnings
	...
	#pragma warning (disable : 4786) // long name debug symbols
#endif
...
#if defined (_MSC_VER) && (_MSC_VER >= 1100) && !defined(SS_NO_BSTR)
	#include <comdef.h>
...
//		}
	...
	// -------------------------------------------------------------------------
	#if defined(SS_WIN32) && !defined(SS_NO_BSTR)
...
#if !defined(SS_ANSI) && !defined(SS_NO_BSTR)
