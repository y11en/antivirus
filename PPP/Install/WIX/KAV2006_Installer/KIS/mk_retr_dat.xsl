						...
						<xsl:when test="name() = 'Id'">
							<xsl:choose>
								<xsl:when test="../@LongName">
									<xsl:attribute name="Id">Retr<xsl:value-of select="../@LongName" />Dir</xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="Id">Retr<xsl:value-of select="../@Name" />Dir</xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template match="wix:Component" mode="internal">
		<Component>
				...
				<xsl:if test="string(.)!=''">
					<xsl:choose>
						<xsl:when test="name() = 'Id'">
							<xsl:choose>
								<xsl:when test="../../@LongName">
									<xsl:attribute name="Id">retr_dat_<xsl:value-of select="../../@LongName" /></xsl:attribute>
									<xsl:attribute name="Guid">$(var.retr_dat_<xsl:value-of select="../../@LongName" />_Guid)</xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name="Id">retr_dat_<xsl:value-of select="../../@Name" /></xsl:attribute>
									<xsl:attribute name="Guid">$(var.retr_dat_<xsl:value-of select="../../@Name" />_Guid)</xsl:attribute>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
			...
			<RemoveFile>
				<xsl:choose>
					<xsl:when test="../@LongName">
						<xsl:attribute name="Id">retr_dat_<xsl:value-of select="../@LongName" />_RemoveFiles</xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="Id">retr_dat_<xsl:value-of select="../@Name" />_RemoveFiles</xsl:attribute>
					</xsl:otherwise>
				</xsl:choose>
