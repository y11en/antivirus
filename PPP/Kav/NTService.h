...
class CNTService
{
	...
	protected:	// data members
		...
		CRITICAL_SECTION m_status_cs;
		
	public:
		BOOL					m_bWinNT;			// TRUE, if this is running on WinNT FALSE on Win95
		BOOL					m_bWinNT4;			// TRUE, if this is running on WinNT 4
...
*/
	public:	// helpers
		...
		LPTSTR			GetLastErrorTextEx(DWORD error, LPTSTR Buf, DWORD Size);
		...
		BOOL			ReportStatus(
							...
							BOOL  bAcceptControls = TRUE // FALSE to disable contols
						);
	...
	public:
		...
		void SetupConsole();
		BOOL SetServiceStartType(DWORD dwStartType, DWORD dwTimeout, DWORD* pWin32Error);
		BOOL RegisterGUI (CHAR* szProductName, BOOL bRegister);
		BOOL CheckServiceAccessRights(DWORD dwServiceAccessRights);
