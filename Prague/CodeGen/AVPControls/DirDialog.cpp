...
static const UINT nSetFolderPath = ::RegisterWindowMessage(_T("AVPControlsDirDialog_SetFolderPath"));
...
// ---
static int DDMessageBox( CWnd *pLocWnd, LPCTSTR lpszText, UINT nType = MB_OK ) {
	CString strTitle = ::LoadLocString( pLocWnd, IDS_DD_TITLE );
	return ::MessageBox( NULL, lpszText, strTitle, nType | MB_OK | MB_ICONEXCLAMATION | MB_TASKMODAL );
//	return pLocWnd->GetTopLevelParent()->MessageBox(lpszText, strTitle, nType | MB_OK | MB_ICONEXCLAMATION );
}


// ---
static void MakeFullDirPath( CString &rcSource, CString &rcBase, CString &rcFull ) {
	int len = rcSource.GetLength();
	if ( len >= 3 && rcSource[0] == L'.' && rcSource[1] == L'.' && rcSource[2] == L'\\' ) {
		CString rcCopy(rcSource);

		rcFull = rcBase;
    len = rcFull.GetLength();
    if (len > 3 && rcFull[len-1] == L'\\') 
			rcFull = rcFull.Left( len - 1 );

		len = rcCopy.GetLength();
		while ( len >= 3 && rcCopy[0] == L'.' && rcCopy[1] == L'.' && rcCopy[2] == L'\\' ) {
			if ( rcFull.GetLength() > 3 ) {
				int nInd = rcFull.ReverseFind( L'\\' );
				if ( nInd >= 0 ) 
					rcFull = rcFull.Left( nInd );
			}
			rcCopy = rcCopy.Mid(3);
			len = rcCopy.GetLength();
		}
	...
	if ( rcSource[0] == L'\\' ) {
		rcFull = rcBase.Left( 2 ) + rcSource;
		return;
	}
	if ( len >= 3 && rcSource[1] == L':' && rcSource[2] == L'\\' ) {
		rcFull = rcSource;
		return;
	}
	rcFull = rcBase + rcSource;
}
...
BEGIN_MESSAGE_MAP(CDlgWnd, CWnd)
  ON_BN_CLICKED(IDC_OPEN, OnOpen)
	ON_WM_SIZE()
	ON_REGISTERED_MESSAGE( nSetFolderPath, OnSetFolderPath )
END_MESSAGE_MAP()

LRESULT CDlgWnd::OnSetFolderPath( WPARAM wParam, LPARAM lParam ) {
	m_strPath = LPCTSTR(lParam);
	return 1;
}
...
void CDlgWnd::OnOpen()
{
    ...
    if (len == 2 && ss[0] == L'\\' && ss[1] == L'\\')
    {
			er = ::LoadLocString(m_pLocaleWnd, IDS_DD_FOLDERNOTVALID);
			::DDMessageBox(m_pLocaleWnd, ss + _T("\n") + er);
			pEdit->SetFocus();
			return;
    }
    else if (len == 0 || len == 1 && ss[0] == L'\\')
    {
        // Current directory or root of the current drive (therefore must be valid)
        ;
    }
    else if ((len == 2 && ss[1] == L':') ||
             (len == 3 && ss[1] == L':' && ss[2] == L'\\') )
    {
        _TCHAR rootdir[4] = _T("?:\\");
        rootdir[0] = ss[0];
        ...
        {
						er = ::LoadLocString(m_pLocaleWnd, IDS_DD_DRIVENOTVALID);
            ::DDMessageBox(m_pLocaleWnd, ss + _T("\n") + er);
            pEdit->SetFocus();
            return;
        }
    ...
    else
    {
        ...
        if (attr == 0xFFFFFFFF)
        {
            ...
            if (len > 1 && ss[1] == L':' && GetDriveType(rootdir) <= DRIVE_NO_ROOT_DIR)
            {
								er = ::LoadLocString(m_pLocaleWnd, IDS_DD_DRIVENOTVALID);
                ::DDMessageBox(m_pLocaleWnd, ss + _T("\n") + er);
                pEdit->SetFocus();
            ...
            else if (len >= 2 && ss[0] == L'\\' && ss[1] == L'\\' && 
                     ( (ss2 = _tcschr((const TCHAR *)ss+2, L'\\')) == NULL || _tcschr(ss2+1, L'\\') == NULL) )
            {
								er = ::LoadLocString(m_pLocaleWnd, IDS_DD_FOLDERNOTVALID);
                ::DDMessageBox(m_pLocaleWnd, ss + _T("\n") + er);
                pEdit->SetFocus();
            ...
            else
            {
                // Appears to be a valid drive (or relative path)
								CString full;
								::MakeFullDirPath(ss, m_strPath, full);
								if ( m_pHost->CheckFolderName(full) )
								{
										CString mess(full);
										er = ::LoadLocString(m_pLocaleWnd, IDS_DD_CREATEFOLDER);
										mess += _T("\n");
										mess += er;
										if (::DDMessageBox(m_pLocaleWnd, mess, MB_YESNO) == IDYES)
										{
												...
												CString current;
												TCHAR *pBuffer = current.GetBuffer( current.GetLength() + 1 );
												::GetCurrentDirectory( current.GetLength() + 1, pBuffer );
												BOOL bResult = ::IOSCreateDirectory( full + _T("\\") );//::MakeSureDirectoryPathExists( full + _T("\\") );
												::SetCurrentDirectory( pBuffer );
												current.ReleaseBuffer();
												if ( !bResult/*!::MakeSureDirectoryPathExists(full + _T("\\"))*/)                   
												{
														switch (GetDriveType(rootdir))
														{
														case DRIVE_CDROM:
																er = ::LoadLocString(m_pLocaleWnd, IDS_DD_CREATEFOLDERCD);
																::DDMessageBox(m_pLocaleWnd, er);
																break;
														case DRIVE_REMOVABLE:
																er = ::LoadLocString(m_pLocaleWnd, IDS_DD_CREATEFOLDERWP);
																::DDMessageBox(m_pLocaleWnd, er);
																break;
														case DRIVE_REMOTE:
																er = ::LoadLocString(m_pLocaleWnd, IDS_DD_CREATEFOLDERPERMNET);
																::DDMessageBox(m_pLocaleWnd, er);
																break;
														default:
																er = ::LoadLocString(m_pLocaleWnd, IDS_DD_CREATEFOLDERPERM);
																::DDMessageBox(m_pLocaleWnd, er);
																break;
														}
														pEdit->SetFocus();
        ...
        {
						er = ::LoadLocString(m_pLocaleWnd, IDS_DD_NOTFOLDER);
            ::DDMessageBox(m_pLocaleWnd, ss + _T("\n") + er);
            pEdit->SetFocus();
            return;
        }
...
//    GetDlgItem(IDC_OPEN)->ModifyStyle(BS_DEFPUSHBUTTON, 0);
//		GetDlgItem(IDOK)->ShowWindow( SW_SHOW );
//		GetDlgItem(IDOK)->EnableWindow( TRUE );
//    GetDlgItem(IDOK)->ModifyStyle(0, BS_DEFPUSHBUTTON);
//		PostMessage( WM_COMMAND, MAKEWPARAM(IDOK, BN_CLICKED), LPARAM(::GetDlgItem(m_hWnd, IDOK)) );
...
void CDlgWnd::OnSize(UINT nType, int cx, int cy) {
  ...
  CButton *pOpen = (CButton *)GetDlgItem(IDC_OPEN);
  ASSERT(pOpen != NULL);

	ASSERT(GetDlgItem(edt1) != NULL);
	GetDlgItem(edt1)->GetWindowRect(rct); //Get edt1 rectangle
	ScreenToClient(rct);

	pEdit->SetWindowPos(NULL, rct.left, rct.top, rct.Width(), rct.Height(), SWP_NOACTIVATE | SWP_NOZORDER);

	ASSERT(GetDlgItem(IDOK) != NULL);
	GetDlgItem(IDOK)->GetWindowRect(rct); //Get OK button rectangle
	ScreenToClient(rct);

	pOpen->SetWindowPos(NULL, rct.left, rct.top, rct.Width(), rct.Height(), SWP_NOACTIVATE | SWP_NOZORDER);
}
...
void CDlgWnd::CheckDir(const CString &ss)
{
    ...
    pOK->SendMessage( WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(1,1) );
    pOK->SendMessage( WM_LBUTTONUP, MK_LBUTTON, MAKELPARAM(1,1) );
    ...
    if (strSaved.IsEmpty() || strSaved[0] == L'.' ||
        strNew.CompareNoCase(strSaved) == 0 || strNew.CompareNoCase(strSaved + _T("\\")) == 0)
    {
...
LRESULT CDirEdit::OnPaste( WPARAM wParam, LPARAM lParam ) {
	if ( OpenClipboard() )  {
...
LRESULT CDirEdit::OnSetFolderPath( WPARAM wParam, LPARAM lParam ) {
	m_strPath = LPCTSTR(lParam);
	return 1;
}
...
void CDirEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    ...
    if (nChar == VK_ESCAPE)
    {
			pp->SendMessage( WM_COMMAND, MAKEWPARAM(IDCANCEL, BN_CLICKED), LPARAM(::GetDlgItem(pp->m_hWnd, IDCANCEL)) );
			//::EndDialog( *pp, IDCANCEL );
			return;
		}
    if (nChar == L'\t')
    {
        ...
        pWnd->SetFocus();                       // Set focus to Open button
			*/
			...
			if ( nShift < 0 )
				hWnd = ::GetNextDlgTabItem(*pp, *this, TRUE);
			else
				hWnd = ::GetNextDlgTabItem(*pp, *this, FALSE);

			if ( NULL != hWnd )							// If there's a control, set focus to it
				::SetFocus( hWnd );
    }
    else if (nChar == L'\r' || nChar == L'\n')
    {
        ...
        if (len == 0 || 
            len == 1 && ss[0] == L'\\' ||
            len >= 2 && ss[0] == L'\\' && ss[1] == L'\\' && _tcschr((const TCHAR *)ss+2, L'\\') == NULL ||
            len == 2 && ss[1] == L':' ||
            len == 3 && ss[1] == L':' && ss[2] == L'\\' )
        {
                    ...
                    GetDriveType(rootdir) > DRIVE_NO_ROOT_DIR)
                {
                    // Appears to be a valid drive (or relative path)
										...
										if ( m_pHost->CheckFolderName(full) )
										{
												...
												{
														CString current;
														TCHAR *pBuffer = current.GetBuffer( _MAX_PATH << 1 );
														::GetCurrentDirectory( _MAX_PATH << 1, pBuffer );
														BOOL bResult = ::IOSCreateDirectory( full + _T("\\") );//::MakeSureDirectoryPathExists( full + _T("\\") );
														::SetCurrentDirectory( pBuffer );
														current.ReleaseBuffer();
														if ( !bResult/*!::MakeSureDirectoryPathExists(full + _T("\\"))*/)                   
        ...
        SetFocus();                         // Make sure caret stays in this edit control
				...
				//pp->GetDlgItem(IDOK)->ShowWindow( SW_SHOW );
				//pp->GetDlgItem(IDOK)->ModifyStyle(0, BS_DEFPUSHBUTTON);
				//pp->SendMessage( WM_COMMAND, MAKEWPARAM(IDOK, BN_CLICKED), LPARAM(::GetDlgItem(pp->m_hWnd, IDOK)) );
				::EndDialog( *pp, IDOK );
    }
    else
    {
        ...
        else if (start == len && nChar != L'\b')
        {
            ...
            if (count == 1)
            {
                ...
                lb_len = ss.ReverseFind(L'\\');
                if (lb_len == -1) lb_len = ss.ReverseFind(L'/');
                if (lb_len == -1) lb_len = ss.ReverseFind(L':');
                if (lb_len == -1)
                    lb_len = ss.GetLength();
                else
...
CDirDialog::CDirDialog(LPCTSTR initial, LPCTSTR filter, CWnd* pParentWnd)
    : CFileDialog(TRUE, NULL, NULL, 
                  ...
                  NULL, pParentWnd),
      m_strPath(initial),
			m_DlgWnd(pParentWnd, this),
			m_Edit(pParentWnd, this)
{
		CString str;
    ...
    if (filter != NULL) {
				str = ::LoadLocString(m_pParentWnd, IDS_DD_FILTER_FOLDERS);
        m_strFilter = str + filter;
		}
    else {
				str = ::LoadLocString(m_pParentWnd, IDS_DD_FILTER_FOLDERS);
        m_strFilter = str;
				str = ::LoadLocString(m_pParentWnd, IDS_DD_FILTER_FILES);
        m_strFilter += str;
		}
    m_strFilter.Replace(L'|', L'\0');
    m_ofn.lpstrFilter = m_strFilter;

    m_ofn.lpstrInitialDir = initial;

		m_strTitle = ::LoadLocString(m_pParentWnd, IDS_DD_TITLE);
...
//---
static BOOL DirExists( LPCTSTR pszDirName ) {
	BOOL bResult = TRUE;
  if ( pszDirName && *pszDirName ) {
		bResult = ::GetFileAttributes(pszDirName) & FILE_ATTRIBUTE_DIRECTORY;
  }
  return bResult;
}

// ---
static void CheckDirExists( LPCTSTR pszDirName ) {
	CPathStr szDir(_MAX_PATH);
	CPathStr szFullDir(pszDirName);
	while ( !::DirExists(szFullDir) ) {
		szDir = pszDirName;
		::PathExtract( LPTSTR(szDir), szFullDir, TCSCLEN(pszDirName) );
	}
}
...
void CDirDialog::OnInitDone()
{
    ...
    ASSERT(pp->GetDlgItem(stc3) != NULL);
		str = ::LoadLocString(m_pParentWnd, IDS_DD_TEXTFOLDER);
    pp->GetDlgItem(stc3)->SetWindowText(str);
    ...
    VERIFY(m_Edit.Create(WS_TABSTOP | WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL,
                           rct, pp, IDC_DIR));
    if (m_ofn.lpstrInitialDir  != NULL) {
			::CheckDirExists(m_ofn.lpstrInitialDir);
...
//  m_Edit.SetSel(0, strlen(m_ofn.lpstrInitialDir));
    ...
    pp->ScreenToClient(rct);
		...
		str = ::LoadLocString(m_pParentWnd, IDS_DD_TEXTOPEN);
    m_Open.Create(str, WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
                    rct, pp, IDC_OPEN);
...
void CDirDialog::OnFolderChange()
{
    ...
    m_Edit.SetSel(len, len);
		...
		pp->SendMessage( nSetFolderPath, 0, LPARAM(LPCTSTR(m_strPath)) );
		m_Edit.SendMessage( nSetFolderPath, 0, LPARAM(LPCTSTR(m_strPath)) );
