//*************************************************************************
// ODMenu.cpp : implementation file
// Version : 2.4
// Date : Feb 11, 1999
// Author : Brent Corkum
// 
...
void CODMenuData::SetWideString(const wchar_t *szWideString)
{
	delete[] m_szMenuText;//Need not check for NULL because ANSI X3J16 allows "delete NULL"
...
void CODMenuData::SetAnsiString(LPCSTR szAnsiString)
{
	ASSERT(szAnsiString);
	USES_CONVERSION;
...
CString CODMenuData::GetString(void)//returns the menu text in ANSI or UNICODE
...
CODMenu::CODMenu(CWnd *pParentWnd/* = NULL*/)
{
...
void CODMenu::SetParentWnd(CWnd* pParentWnd) 
{
	m_pParentWnd = pParentWnd;
}


CWnd* CODMenu::GetParentWnd() 
{
	return m_pParentWnd;
}

BOOL CODMenu::IsMenu(CMenu *submenu)
{
	int m;
...
/*
==========================================================================
void CODMenu::DrawItem(LPDRAWITEMSTRUCT)
---------------------------------------
...
void CODMenu::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(state & MF_SEPARATOR){
		...
		if(lpDIS->itemData != NULL){
			nIconNormal = (((CODMenuData*)(lpDIS->itemData))->menuIconNormal);
			xoffset = (((CODMenuData*)(lpDIS->itemData))->xoffset);
			bitmap = (((CODMenuData*)(lpDIS->itemData))->bitmap);
			strText = ((CODMenuData*) (lpDIS->itemData))->GetString();
...
/*
==========================================================================
void CODMenu::MeasureItem(LPMEASUREITEMSTRUCT)
---------------------------------------------
...
{
	UINT state = (((CODMenuData*)(lpMIS->itemData))->nFlags);
	...
	else
	{
		...
		CWnd *pWnd = m_pParentWnd ? m_pParentWnd : AfxGetMainWnd();            // Get main window
		...
		const wchar_t *lpstrText = ((CODMenuData*)(lpMIS->itemData))->GetWideString();//SK: we use const to prevent misuse
...
void CODMenu::SetIconSize (int width, int height)
{
	m_iconX = width;
	m_iconY = height;
}

BOOL CODMenu::AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,
						   int nIconNormal)
{
	ASSERT(lpstrText);
	USES_CONVERSION;
...
BOOL CODMenu::AppendODMenuW(wchar_t *lpstrText,UINT nFlags,UINT nID,
						   int nIconNormal)
{
	...
	CODMenuData *mdata = new CODMenuData;
...
BOOL CODMenu::ModifyODMenuA(const char * lpstrText,UINT nID,int nIconNormal)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,nIconNormal);//SK: see MFC Tech Note 059
}

BOOL CODMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	int nLoc;
	CODMenuData *mdata;
	
	// Find the old CODMenuData structure:
	CODMenu *psubmenu = FindMenuOption(nID,nLoc);
	if(psubmenu && nLoc>=0)mdata = psubmenu->m_MenuList[nLoc];
	else{
		// Create a new CODMenuData structure:
		mdata = new CODMenuData;
		m_MenuList.Add(mdata);
	}
	ASSERT(mdata);
	if(lpstrText)
	...
	UINT state=GetMenuState(nID,MF_BYCOMMAND);
	mdata->nFlags = state | MF_BYCOMMAND | MF_OWNERDRAW;
	mdata->nID = nID;
	return (CMenu::ModifyMenu(nID,mdata->nFlags,nID,(LPCTSTR)mdata));
}


BOOL CODMenu::ModifyODMenuA(const char * lpstrText,UINT nID,HBITMAP hIconNormal)
{
	USES_CONVERSION;
	return ModifyODMenuW(A2W(lpstrText),nID,hIconNormal);//SK: see MFC Tech Note 059
}

BOOL CODMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,HBITMAP hIconNormal)
{
	int nLoc;
	CODMenuData *mdata;
	...
	if(lpstrText)
		mdata->SetWideString(lpstrText);  //SK: modified for dynamic allocation
	if(hIconNormal){
		mdata->menuIconNormal = -2;
		mdata->xoffset=0;
		if(mdata->bitmap)mdata->bitmap->DeleteImageList();
		else mdata->bitmap=new(CImageList);
		mdata->bitmap->Create(m_iconX,m_iconY,ILC_COLORDDB|ILC_MASK,1,1);
		AddBitmapToImageList(mdata->bitmap,hIconNormal);
	}
	UINT state=GetMenuState(nID,MF_BYCOMMAND);
	mdata->nFlags = state | MF_BYCOMMAND | MF_OWNERDRAW;
	mdata->nID = nID;
...
BOOL CODMenu::ModifyODMenuA(const char *lpstrText,const char *OptionText,
						   int nIconNormal)
{
	USES_CONVERSION;
...
BOOL CODMenu::ModifyODMenuW(wchar_t *lpstrText,wchar_t *OptionText,
						   int nIconNormal)
{
	CODMenuData *mdata;
...
CODMenuData *CODMenu::NewODMenu(UINT pos,UINT nFlags,UINT nID,CString string)
{
	CODMenuData *mdata;
...
BOOL CODMenu::LoadToolbars(const UINT *arID,int n)
{
	ASSERT(arID);
...
BOOL CODMenu::LoadToolbar(UINT nToolBar)
{
	UINT nID;
	BOOL returnflag=FALSE;
	CToolBar bar;
	
	bar.Create(m_pParentWnd ? m_pParentWnd : AfxGetMainWnd());
...
BOOL CODMenu::LoadFromToolBar(UINT nID,UINT nToolBar,int& xoffset)
{
	int xset,offset;
	UINT nStyle;
	BOOL returnflag=FALSE;
...
CODMenu *CODMenu::FindMenuOption(int nId,int& nLoc)
{
	int i;
	CODMenu *psubmenu,*pgoodmenu;
	
	for(i=0;i<(int)(GetMenuItemCount());++i){
#ifdef _CPPRTTI 
		psubmenu=dynamic_cast<CODMenu *>(GetSubMenu(i));
#else
		psubmenu=(CODMenu *)GetSubMenu(i);
#endif
		if(psubmenu){
...
CODMenuData *CODMenu::FindMenuOption(wchar_t *lpstrText)
{
	int i,j;
	CODMenu *psubmenu;
	CODMenuData *pmenulist;
...
BOOL CODMenu::LoadMenu(int nResource)
{
	return(CODMenu::LoadMenu(MAKEINTRESOURCE(nResource)));
};

BOOL CODMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	TRACE(_T(
		"IMPORTANT:Use CODMenu::DestroyMenu to destroy Loaded Menu's\n"));
	ASSERT_VALID(this);
	...
	CODMenuArray  m_Stack;    // Popup menu stack
	CSArray<BOOL>  m_StackEnd;    // Popup menu stack
	...
	do{
		...
		//WideCharToMultiByte
		if(dwFlags & MF_POPUP){
			if(dwFlags & MF_END)m_StackEnd[m_Stack.GetUpperBound()] = TRUE;
			CODMenu* pSubMenu = new CODMenu;
			...
			while(j>=0 && m_StackEnd[j]){
				m_Stack[m_Stack.GetUpperBound()]->InsertSpaces();
				m_Stack.RemoveInd(j);
				m_StackEnd.RemoveInd(j);
				--j;
			}
		}
...
void CODMenu::InsertSpaces(void)
{
	...
	CWnd *pWnd = m_pParentWnd ? m_pParentWnd : AfxGetMainWnd();  
	CDC *pDC = pWnd->GetDC();
...
void CODMenu::LoadCheckmarkBitmap(int unselect, int select)
{
...
BOOL CODMenu::GetMenuText(UINT id, CString& string)
{
	BOOL returnflag=FALSE;
...
void CODMenu::DrawCheckMark(CDC* pDC,int x,int y,COLORREF color)
{
...
CODMenuData *CODMenu::FindMenuList(UINT nID)
{
...
void CODMenu::InitializeMenuList(int value)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i)
		m_MenuList[i]->syncflag=value;
}

void CODMenu::DeleteMenuList(void)
{
	for(int i=0;i<=m_MenuList.GetUpperBound();++i){
...
void CODMenu::SynchronizeMenu(void)
{
	CODMenuDataArray temp;
	CODMenuData *mdata;
	CString string;
...
// --- 
void CODMenu::DitherBlt( CBitmap &srcBmp, CBitmap &dstBmp ) {
	BITMAP bmp;
  ...
  CBitmap * psDCOldBmp = sDC.SelectObject( &srcBmp );
	...
	sDC.SetBkColor( m_bitmapBackground/*GetSysColor(COLOR_BTNFACE)*/ );
	...
	sDC.FillSolidRect( 0,0, nWidth, nHeight, GetSysColor((IsNewShell())?COLOR_3DFACE:COLOR_MENU) );
...
void CODMenu::SynchronizeIcons(void)
{
	...
	if(pWnd)
	{
		...
		for(j=0;j<GetMenuItemCount();++j){
			...
			if(!(state&MF_POPUP) && !(state&MF_SEPARATOR)){
				...
				//GetMenuString(j,string,MF_BYPOSITION);
				HBITMAP hBmp=NULL;
				pWnd->SendMessage(nGetMenuItemBmp, nID, LPARAM(&hBmp));
				if(hBmp) {
					if ( state & MF_GRAYED ) {
						CBitmap bitmapstandard;
						CBitmap bitmapgrayed;
						bitmapstandard.Attach(hBmp);
						DitherBlt( bitmapstandard, bitmapgrayed );
						ModifyODMenuA(NULL,nID,bitmapgrayed);
						bitmapstandard.DeleteObject();
						bitmapgrayed.Detach();
					}
					else
						ModifyODMenuA(NULL/*string*/,nID,hBmp);
				}
			}
		}
	}
...
void CODMenu::UpdateMenu(CMenu *pmenu)
{
#ifdef _CPPRTTI 
	CODMenu *psubmenu = dynamic_cast<CODMenu *>(pmenu);
#else
	CODMenu *psubmenu = (CODMenu *)pmenu;
#endif
	if(psubmenu)psubmenu->SynchronizeMenu();
}

LRESULT CODMenu::FindKeyboardShortcut(UINT nChar, UINT nFlags,
                                     CMenu *pMenu)
{
#ifdef _CPPRTTI 
	CODMenu *pODMenu = dynamic_cast<CODMenu *>(pMenu);
#else
	CODMenu *pODMenu = (CODMenu *)pMenu;
#endif
	if(pODMenu && nFlags&MF_POPUP){
		CString key(_T('&'),2);//SK: modified for Unicode correctness
		key.SetAt(1,(TCHAR)nChar);
		key.MakeLower();
		CString menutext;
		int menusize = (int)pODMenu->GetMenuItemCount();
		if(menusize!=(pODMenu->m_MenuList.GetUpperBound()+1))
			pODMenu->SynchronizeMenu();
		for(int i=0;i<menusize;++i){
			if(pODMenu->GetMenuText(i,menutext)){
				menutext.MakeLower();
...
void CODMenu::DitherBlt (HDC hdcDest, int nXDest, int nYDest, int nWidth, 
...
int CODMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	int bReturn;
...
int CODMenu::AddBitmapToImageList(CImageList *bmplist,HBITMAP hbmp)
{
	int bReturn;
...
void CODMenu::SetBitmapBackground(COLORREF color)
{
	m_bitmapBackground=color;
	m_bitmapBackgroundFlag=TRUE;
}

void CODMenu::UnSetBitmapBackground(void)
{
	m_bitmapBackgroundFlag=FALSE;
}
...
void CODMenu::AddFromToolBar(CToolBar* pToolBar, int nResourceID)
{
...
BOOL CODMenu::Draw3DCheckmark(CDC *dc, const CRect& rc,
...
void CODMenu::DitherBlt2(CDC *drawdc, int nXDest, int nYDest, int nWidth, 
...
void CODMenu::SetDisableOldStyle(void)
{
	disable_old_style=TRUE;
}

void CODMenu::UnSetDisableOldStyle(void)
{
	disable_old_style=FALSE;
}

BOOL CODMenu::GetDisableOldStyle(void)
{
	return(disable_old_style);
}


HBITMAP CODMenu::LoadSysColorBitmap(int nResourceId)
{
	HINSTANCE hInst = 
