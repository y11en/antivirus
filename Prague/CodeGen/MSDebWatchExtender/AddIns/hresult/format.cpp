...
// this table defines known HRESULT values
tName hrs[] = {
	{ 0x0,        "S_OK" },
	{ 0x1,        "S_FALSE" },
...
// this function find value in table and returns appropriate name
					  ...
					  ) 
{
	__try {
		...
		if ( dwValue == 0xccccccccl )
			...
			lstrcpyn( pResult, name, max );

		else if ( FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM,0,dwValue,0,src,sizeof(src),0))
		{
			lstrcpyn( pResult, src, max );
			CHAR* ptr = pResult + lstrlen(pResult)-1;
			while (ptr >= pResult && (*ptr==0x0D || *ptr==0x0A))
			{
				*ptr = 0;
				ptr--;
			}
			...
			lstrcpyn( pResult, "cannot resolve HRESULT", max);

	}
	__except( EXCEPTION_EXECUTE_HANDLER ) {
		lstrcpyn( pResult, "cannot resolve HRESULT, exception", max );
	}
