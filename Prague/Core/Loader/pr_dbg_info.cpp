...
extern "C" tERROR pr_call PrDbgGetCallerInfo(tCHAR* szSkipModulesList, tDWORD dwSkipLevels, tCHAR* pBuffer, tDWORD dwSize)
{
	...
	if (PR_SUCC(error))
	{
		...
		if (PR_SUCC(error))
		{
			...
			if (*szSrcFile)
				_snprintf_s(pBuffer, dwSize, dwSize, "%s(%d) : %s : %s", szSrcFile, dwSrcLine, szModuleName, szFunc);
			else 
				_snprintf_s(pBuffer, dwSize, dwSize, "%s : %08X%s%s", szModuleName, dwAddress, (*szFunc ? " : " : ""), (*szFunc ? szFunc : ""));
			error = errOK;
		}
	}
	...
	if (PR_SUCC(error))
	{
		...
		else
		{
			...
			case DBGINFO_MODULE_NAME:
				strncpy_s(pStrBuffer, dwSize, module.ModuleName, min(dwSize, countof(module.ModuleName)));
				pStrBuffer[dwSize-1] = 0;
				break;
			
			case DBGINFO_IMAGE_NAME:
				strncpy_s(pStrBuffer, dwSize, module.ImageName, min(dwSize, countof(module.ImageName)));
				pStrBuffer[dwSize-1] = 0;
				break;
					...
					else
					{
						...
						if (pSymGetSymFromAddr(hProcess, dwAddress, &dwDisplacement, symbol))
						{
							...
							if (pSymUnDName(symbol, szUnDName, MAX_SYMBOL_NAME_SIZE))
								pSymName = szUnDName;
							strncpy_s(pStrBuffer, dwSize, pSymName, min(dwSize, MAX_SYMBOL_NAME_SIZE));
							pStrBuffer[dwSize-1] = 0;
						}
						else
							error = PR_WIN32_TO_PRAGUE_ERR(GetLastError());
			...
			case DBGINFO_SRC_FILE:
				{
					...
					{
						strncpy_s(pStrBuffer, dwSize, line.FileName, min(dwSize, strlen(line.FileName)+1));
						pStrBuffer[dwSize-1] = 0;
						break;
					}
				}
