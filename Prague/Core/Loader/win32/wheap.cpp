...
tERROR pr_call _InitHeap( hi_Heap heap ) {
	...
	d->m_hHeap = new ptrset;
	ReleaseMutex( create_heap_protector );
	
	PR_TRACE(( heap, prtNOTIFY, "ldr\tPrHeap(0x%p) - WinHeap(0x%p) created", heap, d->m_hHeap ));
...
tERROR pr_call Heap_ObjectInitDone( hi_Heap _this ) {
  ...
  if ( d->m_early_initialized )
	...
	PR_TRACE(( _this, prtNOTIFY, "ldr\tPrHeap(0x%p)/WinHeap(0x%p) created - %terr", _this, d->m_hHeap, error ));
...
tERROR pr_call Heap_ObjectClose( hi_Heap _this ) {
	...
	if ( d->m_hHeap != NULL )	{
		...
		if ( heap )
		{
			ptrset *ps = (ptrset *)heap;
			for(ptrset::iterator it = ps->begin(); it != ps->end(); ++it)
				free(*it);
			delete ps;
		}
		else
	...
	if ( succ ) {
		PR_TRACE(( _this, prtNOTIFY, "ldr\tPrHeap(0x%p)/WinHeap(0x%p) destroyed", _this, heap ));
	}
	else {
...
tERROR pr_call Heap_Alloc( hi_Heap _this, tPTR* result, tDWORD size ) {
  ...
  if ( d->m_hHeap )
	...
	if ( PR_SUCC(error) ) {
		WaitForSingleObject( create_heap_protector, INFINITE );
		ptrset *ps = (ptrset *)d->m_hHeap;
		ret_val = calloc(size, 1);
		if(ret_val) {
			ps->insert(ret_val);
			error = errOK;
		}
...
tERROR pr_call Heap_Realloc( hi_Heap _this, tPTR* result, tPTR ptr, tDWORD size ) {
	...
	if ( PR_SUCC(error) ) {
		...
		if ( PR_SUCC(error) ) {
			...
			if(size > old_size)
				memset((tCHAR *)ret_val + old_size, 0, size - old_size);
			ptrset::iterator it = ps->find(ptr);
			if(it != ps->end())
				ps->erase(it);
			if ( ret_val ) {
				ps->insert(ret_val);
