...
extern tERROR pr_call Timer_Register( hROOT g_root );
...
extern "C" tBOOL __stdcall DllMain( tPTR hInstance, tDWORD dwReason, tERROR* pERROR ) {
	...
	switch( dwReason ) {
		...
		case PRAGUE_PLUGIN_LOAD :
			...
			*pERROR = errOK;
			//resolve  my imports
			//if ( PR_FAIL(*pERROR=CALL_Root_ResolveImportTable(g_root,&count,import_table_variable...,PID_TIMER)) ) {
			//   PR_TRACE(( g_root, prtERROR, "cannot resolve import table for ..." ));
			//   return cFALSE;
			//}

			//register my exports
			//CALL_Root_RegisterExportTable( g_root, &count, export_table_..., PID_TIMER );
			...
			if ( PR_FAIL(*pERROR=Timer_Register(g_root)) ) {
				PR_TRACE(( g_root, prtERROR, "cannot register \"Timer\" interface"));
				return cFALSE;
			}
			break;
