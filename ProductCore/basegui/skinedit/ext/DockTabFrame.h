...
Dec 31,2003; Igor Katrayev:
came up with the first version

Jan 7,2004; Igor Katrayev:
bug fix: added floatRect member initialization in ClientView class constructor.
...
namespace DockSplitTab {
	...
	protected:
		...
		public:
			...
			// Event handlers
			DECLARE_FRAME_WND_CLASS(_T("DockSplitTab::FloatFrame"), 0)
			...
			LRESULT OnNCLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
				...
				UINT hitTest = this->DefWindowProc( WM_NCHITTEST, 0, lParam);
		...
		public:
			...
			// Event handlers
			DECLARE_WND_CLASS(_T("DockSplitTab::DockPane"))
			...
			LRESULT OnSize( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
				...
				case dockTOP:
					...
					this->splitPaneRect = CRect( 0, dy, cx, cy - this->systemCyFrame); dy += this->splitPaneRect.Height();
					this->sliderRect    = CRect( 0, dy, cx, cy);
					break;
					
				case dockBOTTOM:
					
					this->sliderRect    = CRect( 0,  0, cx, this->systemCyFrame);  dy = this->systemCyFrame;
			...
			LRESULT OnPaint( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
				...
				if ( !rect.IsRectEmpty()) {
					...
					CBrush oldBrush = paintDC.SelectBrush( *borderBrush);
					...
					CString str;
					str += this->getCaption();
					HFONT oldFont = paintDC.SelectFont( DockPane::systemCaptionFont);
					...
					rect.left  +=  5;
					rect.right -= 50;
					paintDC.DrawText( str, str.GetLength()
									, rect
					...
					CPen oldPen = paintDC.SelectPen( buttonPen);
...
//			using ClientView::stateFloating;
	...
	public:
		...
		bool toggleClientViewState( HWND clientViewWnd, ClientView::State state) {
		...
		HWND floatClientView( LPRECT rect, ClientView* clientView) {
			...
			floatRect.InflateRect( 7, 27); // TODO calculate more accuratelly
			...
			clientView->state = ClientView::stateFloating;
			return floatFrame->m_hWnd;
		}
		...
		void dragStart( HWND sourceWnd, HWND clientViewWnd, long x, long y, DWORD keysPressed) {
			...
			if ( NULL != clientViewWnd) {
				...
				trackRect = NULL == clientView
						 ...
						 || clientView->floatRect.EqualRect( &rectNULL)
				...
				} else if ( hitPoint.x <= clientViewRect.left) {
					
					offset = clientViewRect.left - hitPoint.x;
...
dragOver_exit:
		...
		void clientActivate(  HWND childWnd, HWND clientViewWnd) {
			...
			SplitPane* splitPane = ( NULL != clientViewWnd)
			...
			for ( int i=0; i<4; i++) {
				...
				if ( NULL != dockPane) {
					...
					bool active = dockPane->getPane() == splitPane;
					if ( active) {
			...
			if ( !activated) {
				...
				if ( NULL != position)
					do {
						...
						FloatFrame* floatFrame = this->floatFrames.GetNextValue( position);
						if ( activated = ( floatFrame->getPane() == splitPane)) {
		...
		// Event handlers
		DECLARE_WND_CLASS(_T("DockSplitTab::Frame"))
		...
		LRESULT OnSetFocus( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
			...
			if ( currentPane != &this->mainPane) {
				...
				if ( i>=4)
					currentPane = this->lastPane; // the only purpose why we need to keep the last pane
			}
			...
			if ( NULL != dropParcel->clientViewWnd) {
				...
				case placeFLOATFRAME:
					...
					if ( NULL == targetFloatFrame) {
						...
						frameClientRect.InflateRect( 7, 27); // TODO calculate more accuratelly
