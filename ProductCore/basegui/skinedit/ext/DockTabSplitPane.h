...
#define WM_USER_DESTROY_PANE WM_USER+1
	...
	protected:
		...
		void splitWithCurrentTab( Pane* sourcePane, Pane* targetPane, Pane::TabPaneHitTest where) {
			...
			case Pane::TabPaneHitTest_Left: {
					...
					verSplitter->Create( targetPaneParent, &rectNULL, NULL, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0/*WS_EX_CLIENTEDGE*/);
			...
			case Pane::TabPaneHitTest_Bottom: {
					...
					horSplitter->Create( targetPaneParent, &rectNULL, NULL, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, 0/*WS_EX_CLIENTEDGE*/);
	...
	public:
		...
		int getClientViewCount() {
			return this->clientViewPaneMap.GetCount();
		}
		...
		// Event handlers
		DECLARE_WND_CLASS(_T("DockSplitTab::SplitPane"))
