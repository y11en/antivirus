	...
	void DoPrePaint(RECT rcClient, LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		else
		{
			...
			if(CTCS_BOTTOM == (dwStyle & CTCS_BOTTOM))
			{
				...
				rcTmp = rc;
				rcTmp.bottom = rcTmp.top + 3;
				dc.FillSolidRect(&rcTmp, lpNMCustomDraw->clrBtnFace);
				...
				*/
				...
				rcTmp = rc;
				rcTmp.top += 3;
				dc.DrawEdge(&rcTmp, BDR_SUNKENOUTER, BF_RECT);
			}
			else
			{
				RECT rcTmp;
				...
				dc.MoveTo(rcTmp.left, rcTmp.top-1);
				dc.LineTo(rcTmp.right, rcTmp.top-1);
