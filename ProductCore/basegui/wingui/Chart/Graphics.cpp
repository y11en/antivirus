...
CGraphics::CGraphics()
{
	...
	m_nGridColor	= RGB(225, 225, 225);
	m_nBorderColor	= m_nGridColor;//RGB(0, 0, 0);
...
void CGraphics::RecalcRects(RECT& rt)
{
	...
	m_ClientRect = rt;
	SetPixelRect(rt);
	m_bM = 16 + (m_Rect.bottom - m_Rect.top) / 16;
	m_tM = 8 + (m_Rect.bottom - m_Rect.top) / 22;
	m_lM = 10 + (m_Rect.right - m_Rect.left) / 16;
	m_rM = 1;
...
//				m_ClientRect.right, m_ClientRect.bottom);
	::FillRect(m_hDC, &m_ClientRect, hBrush);
...
*/
		int xb = GR + 2 * n;
		int xe = xb + 2 * n;
		int yb  = GT - 3 * Index * m_LogFont.lfHeight / 2;
		int ye = yb + 2;
		DrawRectangle(xb, yb, xe, ye);		

	  	int bm  = ::SetBkMode(m_hDC, TRANSPARENT);
		HFONT hOldFont = (HFONT)::SelectObject(m_hDC, m_Font);
		::SetTextColor(m_hDC, cr); 
		SetStringAlign(LEFT, CENTER);
		PrintString(xe + n, yb, 0, Name);
		::SelectObject(m_hDC, hOldFont);
		::DeleteObject(m_Font);
		::SetBkMode(m_hDC, bm);
	}
...
//	RECT textRect = {x,y,x+30,y+30};
//	::DrawText(m_hDC, fmt, lstrlen(fmt), &textRect, 0);
...
//		if((i % j) == 0) 
		{
			xb = GL - m_bM / 7;
			value = (float)(m_Scale.ymax - i * (m_Scale.ymax - m_Scale.ymin) / YGridTicks);
			Format(m_nYDecimal, str, value);
			PrintString(GL - m_bM / 5, yb, 0, str);
		}
//		else
//			xb = GL - m_lM / 10;
...
//	if (m_bEnableLegend)
	...
	if (m_nAxesType == XY || m_nAxesType == XLOG)
	{
		...
		{
			yb = ye = (int)(GT + 1.0 * PY * i / YTicks );
			// each 10-th point - label is drew
			if((i % j) == 0) 
			{
				// width - large
				xe = GR + m_rM / 10;
		...
		for(i = 1; i <= m_nYStep; i ++)
		{
			for(j = 1; j <= 10; j ++)
			{
				if (k == 0)   
					...
					xe = GR + m_rM / 20;
				yb = ye = GB - (int)(log10((double)j) * PY / m_nYStep + 1.0 * (i - 1) * PY / m_nYStep);
				if (j == 1)
				{
...
void CGraphics::TestPrint(int x, int y, char* str, bool marker)
{
	// font size //
	...
	if (m_Font)
	{
	  	int bm  = ::SetBkMode(m_hDC, TRANSPARENT);
		HFONT hOldFont = (HFONT)::SelectObject(m_hDC, m_Font);
		::SetTextColor(m_hDC, /*m_nTitleColor*/ RGB (0,0,128)); 
		SetStringAlign(CENTER, CENTER);

		//PrintString((GL + GR) / 2, (m_Rect.top + GT ) / 2, 0, str);

		PrintString(x+15, 
					y-10, 
					0, str);
		if (marker)
		DrawMarker(x,y,FTRIANGLE, 5);
		//CROSS, XCROSS, STAR, CIRCLE, TRIANGLE, DIAMOND, SQUARE, FCIRCLE, FTRIANGLE, FDIAMOND, FSQUARE
