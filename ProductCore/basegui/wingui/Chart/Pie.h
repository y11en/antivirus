...
void CPie<T>::CutPie(T* percentvalues, int numvalues, string* labels, bool offsetflag)
{
	...
	if (m_Font)
	{
		...
		{    
			...
			PieceOfPie(x, y, radius, percentvalues[i],
		                 ...
		                 double AngleS, double AngleE, bool offsetflag)
{
	...
	{
		if (abs(radius*sin(diff)) > abs(cos(diff/2)) || abs(AngleE - AngleS)>3.14159)
    ...
    sprintf(buffer, "%d", (unsigned int)showpercentage);
