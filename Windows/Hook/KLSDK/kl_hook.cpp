...
CKl_HookModule::GetBase(char* ModuleName)
{
    ...
    // перебираем модули и ищем нужный.
    for ( ULONG i = 0; i < pInfoBuff->NumberOfDrivers; ++i ) 
    {
        ULONG   len = (ULONG)strlen ( pInfoBuff->Drivers[i].PathName );
        
        // имя может состоять не только из имени, но и из говна+имя. Нам нужно тольки имя.
...
KLSTATUS
CKl_HookModule::HookFunction(char* fName, PVOID NewFunc, PVOID* pOldFunc)
{
    if ( m_base )
    {
        PIMAGE_DOS_HEADER       pDosHeader;
        ...
        functions   = (DWORD*) ( m_base + (DWORD_PTR)exportDir->AddressOfFunctions );
        ordinals    = (WORD*)  ( m_base + (DWORD_PTR)exportDir->AddressOfNameOrdinals );
        name        = (PCHAR*) ( m_base + (DWORD_PTR)exportDir->AddressOfNames );
        ...
        for ( DWORD i = exportDir->Base; i < exportDir->NumberOfFunctions; ++i ) 
        {
            ...
            if ( 0 == strcmp ( m_base + (DWORD_PTR)name[ i ], fName ) )
            {
                *pOldFunc = m_base + functions[ ordinals[i] ];                
                functions[ ordinals[i] ] = (DWORD) ( (DWORD_PTR)NewFunc - (DWORD_PTR)m_base );
                break;
            }
        }        
...
CKl_ModLoader::PrepareForExec(PVOID *StartEntry)
{
    ...
    *StartEntry      = NULL;
    ...
    m_DriverBase = (char*)KL_MEM_ALLOC( PEOptionalHeader->SizeOfImage );
    ...
    // загрузили секции. Нужно настроить reloc-и
