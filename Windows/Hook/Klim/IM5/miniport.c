    ...
    do
    {
        ...
        if ( STATUS_SUCCESS == SendIoctl( BASE_GET_VERSION, NULL, 0,  &KlickVer, sizeof ( ULONG ), &dwRet ) )
        {
            if ( KlickVer >= 1 )
            {
                ...
                SendIoctl( KLICK_ATTACH_IM, DeviceName.Buffer, DeviceName.MaximumLength,  NULL, 0, &dwRet );
            }
        }
        */
...
*/
                ...
                KlimPacket = OutNdisPkt_Allocate( MyPacket, pAdapt  );
                ...
                {
                    DbgPrint( "Unable to OutNdisPkt_Allocate\n" );
                }
...
--*/
{
    ...
    {  
        ...
        RtlZeroMemory( &pAdapt->Request, sizeof( pAdapt->Request ) );
...
--*/
{
    ...
    PKLIM_PACKET p = (PKLIM_PACKET)MiniportReceiveContext;
...
*/
    ...
    NdisCopyFromPacketToPacket( Packet, 0, BytesToTransfer, ndis_pkt, ByteOffset, BytesTransferred );
...
--*/
{
    ...
    //
// #pragma prefast(suppress: 11, "pAdapt cannot be NULL")
              ...
              IN UINT BufferSize )
{
    ...
    RtlZeroMemory( &Adapt->MyRequest, sizeof ( Adapt->MyRequest ) );

    Adapt->MyRequest.RequestType = RequestType;
    Adapt->MyRequest.DATA.SET_INFORMATION.Oid = Oid;
    Adapt->MyRequest.DATA.SET_INFORMATION.Oid = Oid;
    Adapt->MyRequest.DATA.SET_INFORMATION.InformationBuffer = pBuffer;
    Adapt->MyRequest.DATA.SET_INFORMATION.InformationBufferLength = BufferSize;
    NdisResetEvent( &Adapt->MyRequestEvent );
    
    NdisRequest( &Status, Adapt->BindingHandle, &Adapt->MyRequest );

    if ( Status == NDIS_STATUS_PENDING )
    {
        NdisWaitEvent( &Adapt->MyRequestEvent, 0 );
    }
...
IsLocalMac( char Mac[6] )
{
    ...
    for (ppCursor = &pAdaptList; *ppCursor != NULL; ppCursor = &(*ppCursor)->Next)
    {
        ETH_COMPARE_NETWORK_ADDRESSES_EQ( (*ppCursor)->CurrentMacAddr, Mac, &bRes );
