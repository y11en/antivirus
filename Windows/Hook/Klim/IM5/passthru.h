...
#define LINKNAME_STRING     L"\\DosDevices\\klim5"
#define NTDEVICE_STRING     L"\\Device\\klim5"
...
//
#define KLIM_PKT_GENERIC    'gnrk'
#define KLIM_PKT_HAND_MADE  'hmad'

typedef struct _SEND_RSVD
{
    PNDIS_PACKET    OriginalPkt;
    ULONG           Tag;
    struct _SEND_RSVD*     Next;
    NDIS_HANDLE     SendHandle;
} SEND_RSVD, *PSEND_RSVD;

typedef struct _RcvRsvd
{
    PVOID pKlimPacket;
    ULONG Tag;
} RCV_RSVD, *PRCV_RSVD;
...
typedef struct _ADAPT
{
    ...
    NDIS_REQUEST                   MyRequest;   // собственные request-ы
    NDIS_EVENT                     MyRequestEvent;
    ...
    NPAGED_LOOKASIDE_LIST       InBufList;   
