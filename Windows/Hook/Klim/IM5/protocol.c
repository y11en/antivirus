...
NTSTATUS
AllocatePktLists( PADAPT pAdapt )
{
    ExInitializeNPagedLookasideList( 
        ...
        NULL, 
        0, 
        sizeof ( KLIM_PACKET ) + sizeof ( KLIM_SEND_PACKET ),
        '1kpg',
        0
        );
    ...
    ExInitializeNPagedLookasideList( 
        ...
        NULL, 
        0, 
        sizeof ( KLIM_PACKET ) + sizeof ( KLIM_RECV_NDIS_PACKET ),
        '2kpg',
        0
        );
    ...
    ExInitializeNPagedLookasideList( 
        ...
        NULL, 
        0, 
        sizeof ( KLIM_PACKET ) + sizeof ( KLIM_RECV_BUFF_PACKET ) + pAdapt->MaxFrameSize,
        '3kpg',
        0
        );
...
// #pragma prefast(suppress: 2010, "Ndis guarantees MediumIndex to be within bounds");
        ...
        NdisInitializeEvent( &pAdapt->MyRequestEvent );

        if ( NDIS_STATUS_SUCCESS != MyNdisRequest( 
            pAdapt, 
        ...
        MyNdisRequest( 
            ...
            pAdapt->CurrentMacAddr,
            sizeof ( pAdapt->CurrentMacAddr ) );

        AllocatePktLists( pAdapt );
...
--*/
{
    ...
    if ( NdisRequest == &pAdapt->MyRequest )
    {
        NdisSetEvent( &pAdapt->MyRequestEvent );
        return;
    }
...
--*/
{
    ...
    if ( SendRsvd->Tag == KLIM_PKT_HAND_MADE )
    {
        PNDIS_PACKET    NdisPacket = Packet;
...
{
    PADAPT      pAdapt =(PADAPT)ProtocolBindingContext;
    PRCV_RSVD rsvd = (PRCV_RSVD)Packet->ProtocolReserved;

    PKLIM_PACKET KlimPacket = rsvd->pKlimPacket;

    PNDIS_BUFFER ndis_buf;

    if (! ( ((PRCV_RSVD)Packet->ProtocolReserved)->Tag == 'pmlk' ) )
    {
    ...
    NdisUnchainBufferAtFront( Packet, &ndis_buf );

    NdisFreeBuffer( ndis_buf );
    NdisFreePacket( Packet );

    if ( KlimPacket )
    {
        PKLIM_RECV_BUFF_PACKET pkt = (PKLIM_RECV_BUFF_PACKET)KlimPacket->Packet;
...
*/
                KlimPacket = InBuffPkt_Allocate2( MyPacket, pAdapt );
                ...
                if ( KlimPacket )
                {
                    ...
                    /*
                    if ( NdisGetPacketFlags(Packet) & NDIS_FLAGS_IS_LOOPBACK_PACKET )
                    {
                        KlimPacket->Status = KLIM_PKT_BLOCK;
                    }
                    else
                    */
                    {
        ...
        {
            KlimPacket = InBuffPkt_Allocate( HeaderBuffer, HeaderBufferSize, NULL, PacketSize, pAdapt );
            ...
            while ( KlimPacket )
            {
                ...
                if ( Status != NDIS_STATUS_SUCCESS  )
                {
                    ExFreeToNPagedLookasideList( &pkt->Adapt->InBufList, KlimPacket );
                    return NDIS_STATUS_FAILURE;

                }

                NdisAllocateBuffer( &Status, &ndis_buff, InPktBufferPool, pkt->LookAheadBuffer, pAdapt->MaxFrameSize - pkt->HeaderBufferSize );
                ...
                NdisAdjustBufferLength( ndis_buff, pAdapt->MaxFrameSize - pkt->HeaderBufferSize );                

                NdisChainBufferAtFront( ndis_pkt, ndis_buff );

                ((PRCV_RSVD)ndis_pkt->ProtocolReserved)->pKlimPacket = KlimPacket;
                ((PRCV_RSVD)ndis_pkt->ProtocolReserved)->Tag = 'pmlk';
                ...
                if ( Status != NDIS_STATUS_PENDING )
                {
                    PtTransferDataComplete( pAdapt, ndis_pkt, Status, BytesTransferred );
                }
        ...
        {
            KlimPacket = InBuffPkt_Allocate( HeaderBuffer, HeaderBufferSize, LookAheadBuffer, LookAheadBufferSize, pAdapt );
...
*/
    ...
    if (Status == NDIS_STATUS_SUCCESS)
    {
        ...
        NDIS_PER_PACKET_INFO_FROM_PACKET( MyPacket, ScatterGatherListPacketInfo ) = 
        NDIS_PER_PACKET_INFO_FROM_PACKET( Packet, ScatterGatherListPacketInfo );
/*
        if (pAdapt->MiniportHandle != NULL)
        {
            NdisMIndicateReceivePacket(pAdapt->MiniportHandle, &MyPacket, 1);
        ...
        return((Status != NDIS_STATUS_RESOURCES) ? 1 : 0);
*/
        // Если Status == NDIS_STATUS_RESOURCES, то пакет держать нельзя. нужно скопировать данные в 
        // свой пакет.
        // Если Status != NDIS_STATUS_RESOURCES, то пакет можно подержать подольше, и данные не копировать.

        if ( Status != NDIS_STATUS_RESOURCES )
        {
            KlimPacket = InPkt_Allocate( MyPacket, pAdapt );

            if ( KlimPacket )
            {   
                ((PKLIM_RECV_NDIS_PACKET)(KlimPacket->Packet))->OrigPacket = Packet;
    ...
    )
{
    ...
    if (pAdapt->RefCount == 0)
    {
        ...
        MPFreeAllPacketPools (pAdapt);;
        NdisFreeSpinLock(&pAdapt->Lock);
        ExDeleteNPagedLookasideList(&pAdapt->InPktList);
        ExDeleteNPagedLookasideList(&pAdapt->OutPktList);
        ExDeleteNPagedLookasideList(&pAdapt->InBufList);
