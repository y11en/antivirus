...
void __stdcall SetCurrentHook(HOOK* pHook)
{
...
{
	return IsAddressHookedBy(((*(DWORD**)HookObj)+MethodNum), pHookFunc);
} 
		...
		while ( 1 )
		{
			...
			while ( 1 ) // Loop forever (or until we break out)
			{
				...
				for(pHookTblEntry = pHookTbl; pHookTblEntry->HookProc != NULL; pHookTblEntry++)
				{
					...
					if (bTargetFunc) 
					{
						void* data = &thunkIAT->u1.AddressOfData;
						//ODS(("data=%08X", data));
						EnterCriticalSection(&g_hooks_critical_section);
						if (!IsAddressHookedBy(data, (void*)HookProc))
						{
							...
							if (pHook)
							{
								...
								if (thunk->u1.Ordinal & IMAGE_ORDINAL_FLAG)
								{
									ODS(("Hooked Import: %s->%s:%d", szModuleName, (BYTE*)hModule+importDesc->Name, IMAGE_ORDINAL(thunk->u1.Ordinal)));
								}
								else
								{
									ODS(("Hooked Import: %s->%s:%s", szModuleName, (BYTE*)hModule+importDesc->Name, pOrdinalName->Name));
								}
							}
							//ODS(("Hooked"));
						}
					...
					for ( i=0; i < exportDir->NumberOfFunctions; i++ )
					{
						...
						{					
							...
							if (pHook)
							{
								...
								pHook->pUserData = pHookUserData;
								pHook->dwFlags |= HI_DONT_FREE_ON_RELEASE;
								ODS(("Hooked Export: %s->%s:%s", szModuleName, (BYTE*)hModule+exportDir->Name, (BYTE*)hModule+(DWORD_PTR)name[j]));
							}
						}
...
UINT GetPrologueBytes(IN PVOID OriginalFunc, UINT nRequiredBytes)
{
	...
	__try
	{
		...
		while (nPrologueBytes < nRequiredBytes || bPrefix)
		{
			nOpcodeLen = 0;
			bPrefix = FALSE;

			if (pCode[0] >= 0x40 && pCode[0] <= 0x4f)
			{
				nOpcodeLen = 1;			// inc/dec e##/REX prefix
				bPrefix = TRUE;
			}
			else if (pCode[0] >= 0x50 && pCode[0] <= 0x5f)
			{
				nOpcodeLen = 1;			// push/pop e##/r##
			}
			else if (pCode[0] == 0x81)	// group 1 opcodes
			{
				nOpcodeLen = 6;
			}
			else if (pCode[0] == 0xff)	// group 5
			{
				if (((pCode[1] >> 3) & 0x07) == 0x06/*reg field of ModRM*/)	// push Ev
					nOpcodeLen = 2;
			}
			else if (pCode[0] >= 0x88 && pCode[0] <= 0x8b)		// mov
			{
				nOpcodeLen = 2;
			}
			else if (pCode[0] == 0x68)	// push imm32
			{
				nOpcodeLen = 5;
			}
...
{
	DWORD OldProtect;
// bugs 19587, 19931 fix - signed comparison!
	if (*(PUCHAR)pContext->m_OriginalFunc != 0xE9)
