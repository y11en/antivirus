...
#define _RESOURCE_LOCK_R(_lock) {KeEnterCriticalRegion(); ExAcquireResourceExclusiveLite(_lock, FALSE);}
#define _RESOURCE_LOCK_W(_lock) {KeEnterCriticalRegion(); ExAcquireResourceExclusiveLite(_lock, TRUE);}
#define _RESOURCE_UNLOCK(_lock) {ExReleaseResourceLite(_lock); KeLeaveCriticalRegion();}
