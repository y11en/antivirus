...
//	size_t nCurrentDate;       // current date for LRU tracking
	tFileSize nDataSize;          // current data file size. if (nDataSize < nMaximumSize)next_range_offset = nDataSize;
	tFileSize nMaximumSize;       // file size limit. If 0 - no limit
...
static OBJID_VOLUME_DATA* GetVolumeData(OBJID_CTX* pObjIdCtx, const BYTE VolumeId[VOLUME_ID_SIZE], bool bAdd)
{
	...
	if (!pItem) 
	{
		if (!bAdd)
			return NULL;
		// not found - add
		if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(ID_LIST_ITEM), &pItem, '1him'))
			return NULL;
		if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(OBJID_VOLUME_DATA), &pVolumeData, '2him'))
		{
	...
	)
{
	...
	size_t dwOffset;
	size_t dwBlockSize;
	ID_LIST_ITEM** ppOldestItem = NULL;
	...
	if (!pItem) 
	{
		...
		dwBlockSize = pObjIdCtx->nRecordSize * pObjIdCtx->nRecordsPerCluster;

		if (pObjIdCtx->nMaximumSize != 0 && pObjIdCtx->nDataSize > pObjIdCtx->nMaximumSize)
		{
			// find oldest range
			ID_LIST_ITEM* pVolume = pObjIdCtx->VolumeListHead;
			while (pVolume)
			{
				ID_LIST_ITEM** ppItem;
				size_t i;
				...
				{
					ppItem = &pVolumeData->pRangesList[i];
					pItem = *ppItem;
					while (pItem)
					{
						if (!ppOldestItem || pItem->LastUsed < (*ppOldestItem)->LastUsed)
							ppOldestItem = ppItem;
						ppItem = &pItem->pNext;
						pItem = *ppItem;
					}
		...
		if (ppOldestItem)
		{
			...
			pItem = *ppOldestItem;
			*ppOldestItem = pItem->pNext;
		}
		else
		{
			// not found - add
			if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(ID_LIST_ITEM), &pItem, '3him'))
				return 0;
			// allocate new range at the end of file
			pItem->dwData = pObjIdCtx->nDataSize;
			pObjIdCtx->nDataSize += dwBlockSize;
		}
		...
		while (dwBlockSize)
		{
			size_t size = min(dwBlockSize, sizeof(zero_buff));
	...
	pItem->LastUsed = OBJID_GetCurrentDate(); //pObjIdCtx->nCurrentDate;
...
static HRESULT SaveIndex(OBJID_CTX* pObjIdCtx)
{
	...
	IndexHdr.Magic = (size_t) GetTimeFast();
...
static HRESULT LoadIndex(OBJID_CTX* pObjIdCtx)
{
	...
	{
		if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(ID_LIST_ITEM), &pItem, '4him'))
			return ERROR_NOT_ENOUGH_MEMORY;
		if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(OBJID_VOLUME_DATA), &pVolumeData, '5him'))
			return ERROR_NOT_ENOUGH_MEMORY;
		...
		{
			int j;
			for (j=0; j<countof(pVolumeData->pRangesList); j++)
			{
				ID_LIST_ITEM* pItem2;
				size_t count;
				...
				for (; count>0; count--)
				{
					if (!pObjIdCtx->pHeap->Alloc(pObjIdCtx->pHeap, sizeof(ID_LIST_ITEM), &pItem2, '6him'))
						return ERROR_NOT_ENOUGH_MEMORY;
					...
					if (!pObjIdCtx->pFileIndex->Read(pObjIdCtx->pFileIndex, &pItem2->dwData, sizeof(pItem2->dwData), NULL))
						return ERROR_READ_FAULT;
					pItem2->pNext = pVolumeData->pRangesList[j];
					pVolumeData->pRangesList[j] = pItem2;
				}
			}
		}
	}
...
HRESULT OBJID_Init(tHeap* pHeap, tFile* pFileData, tFile* pFileIndex, size_t dwRecordSize, OBJID_CTX** ppObjIdCtx, ULONG* pCurrentItemSize)
{
	...
	if (!pHeap->Alloc(pHeap, sizeof(OBJID_CTX), &pObjIdCtx, '7him'))
		return ERROR_NOT_ENOUGH_MEMORY;
	...
	)
{
	...
	dwOffset = GetObjectDataOffset( pObjIdCtx, VolumeId, ObjectId, bResetRequest ? FALSE : TRUE );
...
unsigned __int64 OBJID_GetCurrentDate()
{
	LARGE_INTEGER time;
	KeQuerySystemTime(&time);
	return (unsigned __int64)time.QuadPart;
}
