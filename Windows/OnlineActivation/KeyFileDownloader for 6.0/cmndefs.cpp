...
InstanceData::ValueType InstanceData::instance;
...
CAppData::CAppData() :
					...
					m_lpszEmail(NULL),
					m_lpszWww(NULL),
					m_bUseIeSettings(FALSE),
					m_bUseProxy(FALSE),
					m_bTryAutoAuth(FALSE),
					m_lpszProxyList(NULL),
					...
					)
{
	...
	SetStringParm(m_lpszServerUrl, lpszServerUrl);
	...
	m_bUseIeSettings = bUseIeSettings;
	m_bUseProxy = bUseProxy;
	m_bTryAutoAuth = bTryAutoAuth;

	SetStringParm(m_lpszProxyList, lpszProxyList);
	SetStringParm(m_lpszProxyBypass, lpszProxyBypass);
}
...
void SendDlgMsg(HWND hDialog, int nDlgItem, int nMsg)
{
	...
	{
		InstanceData::Instance().callback.Call(nMsg);
	}
	else
	{
