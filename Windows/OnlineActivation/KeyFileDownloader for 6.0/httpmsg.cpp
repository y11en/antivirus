...
CHttpMessage::CHttpMessage(LPCTSTR szServerUrl, CInetSession& inetSession, CAppData *pAppData)
	: m_pAppData(pAppData)
	, m_hConnection(NULL)
	, m_hRequest(NULL)
	, m_dwContext(NULL)
{
	...
	if (InternetCrackUrl(szServerUrl, lstrlen(szServerUrl), 0, &urlComp))
	{
		m_resource = urlComp.lpszUrlPath;
		// open internet connection
		m_hConnection = InternetConnect
		(
			inetSession,
			urlComp.lpszHostName,
			urlComp.nPort ? urlComp.nPort : INTERNET_INVALID_PORT_NUMBER,
...
LPCTSTR CHttpMessage::GetResource() const
{
...
DWORD CHttpMessage::SendRequest(CReqestEntity &requestEntity)
{
	...
	m_hRequest = HttpOpenRequest(
						...
						INTERNET_FLAG_PRAGMA_NOCACHE | 
							...
							(m_pAppData->m_bTryAutoAuth ? 0 : INTERNET_FLAG_NO_AUTH),
						m_dwContext
					);
	// check for errors
	if (!m_hRequest)
...
{
	if (InstanceData::Instance().stop_flag && m_hConnection != NULL)
	{
...
DWORD CHttpMessage::SetProxyAuth(const TCHAR* szUserName, const TCHAR* szPassword)
{
	if (!m_hConnection)
		return ERROR_INVALID_HANDLE;
