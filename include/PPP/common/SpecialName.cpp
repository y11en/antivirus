...
int MakeSpecialName(LPTSTR strBuffer, DWORD nBufferSizeInTChars,
						LPCTSTR suffix1, LPCTSTR suffix2, bool bNoDateTime /*= false*/)
{
	...
	SYSTEMTIME st;
	return bNoDateTime ?
		_stprintf_s(strBuffer + _tcslen(strBuffer),
			...
			suffix2)
		:
		(GetLocalTime(&st), _stprintf_s(strBuffer + _tcslen(strBuffer),
			nBufferSizeInTChars - _tcslen(strBuffer),
