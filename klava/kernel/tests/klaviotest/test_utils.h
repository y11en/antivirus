...
enum // argument types
{
	...
	ARG_TYPE_BOOL,  // 0/1, true/false, yes/no, present/absent
};
...
{
public:
	Command_Parser (const COMMAND_DESCRIPTOR *cmds = 0, unsigned int cnt = 0)
		: m_commands (cmds), m_command_count (cnt), m_current_cmd (0) {}
	
	void init (const COMMAND_DESCRIPTOR *cmds, unsigned int cnt)
		{ m_commands = cmds; m_command_count = cnt; }
