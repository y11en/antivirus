...
typedef klavstl::trivial_vector <TestObject, klavstl::malloc_allocator> test_vector_t;
...
bool trivial_vector_test ()
{
	int leak_flag = 0;
	{
		KLAV_HEAP_CHECKER (heap_checker, "klavstl::trivial_vector<> : memory leaks detected\n", & leak_flag);
		...
		test_vector_t vec3 (vec1.begin (), vec1.end ());
