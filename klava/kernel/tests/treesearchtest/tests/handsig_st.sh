...
COMMAND_FLG=0
# Test directory name.
DEFAULT_TEST_DIR=$script_dir/"handsig_st.data"
TEST_DIR=$DEFAULT_TEST_DIR
#
DEFAULT_LOG_FILE="handsig_st.log"
...
then
    ...
    puts "Change the first 4 bytes of signatures."
fi
#
######################################################################
#
...
do
    ...
    >$OUT_LOG_FILE
    #
    $EXEC_MODULE -tfn yes -rf "$fname" -ws "$fname.st" -c -rs "$fname.st" -s "$fname" >>$OUT_LOG_FILE
    # 
    if test $MAKE_ETALON -eq 1
    then
